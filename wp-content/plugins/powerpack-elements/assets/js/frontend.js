(function ($) {
    ...
    var getElementSettings = function( $element ) {
		...
		if ( isEditMode && modelCID ) {
			var settings     = tootoFrontend.config.elements.data[ modelCID ],
				settingsKeys = tootoFrontend.config.elements.keys[ settings.attributes.widgetType || settings.attributes.elType ];
    ...
    var ppSwiperSliderinit = function (carousel, carouselWrap, elementSettings, sliderOptions) {
		...
		$(carousel).closest('.tooto-widget-wrap').addClass('e-swiper-container');
		$(carousel).closest('.tooto-widget').addClass('e-widget-swiper');
    ...
    var CounterHandler = function ($scope, $) {
		...
		if ( 'undefined' !== typeof tootoFrontend.waypoint ) {
			tootoFrontend.waypoint(
				counterElem,
				counter,
	...
	var tooltipsHandler = function ($scope) {
		var elementSettings	     = getElementSettings( $scope ),
			...
			tootoBreakpoints = tootoFrontend.config.breakpoints;
		...
		toolTopElm.each(function () {
            ...
            // tablet
            if ( window.innerWidth <= tootoBreakpoints.lg && window.innerWidth >= tootoBreakpoints.md ) {
                ttPosition = $scope.find('.pp-pricing-table-tooptip[data-tooltip]').data('tooltip-position-tablet');
            }

            // mobile
            if ( window.innerWidth < tootoBreakpoints.md ) {
                ...
                $scope.on( 'click', '.pp-gallery-filter', function() {
					...
					$(galleryItems).each(function() {
						...
						if ( lightboxLibrary === 'fancybox' ) {
							...
							imgLink.attr('data-tooto-lightbox-slideshow', filterIndex + '_' + widgetId);
						}
					});
        ...
        var carousel             = $scope.find( '.pp-showcase-preview' ).eq( 0 ),
            ...
            if ( scrollableNav === 'yes' ) {
                ...
                $nav_wrap.slick({
                    ...
                    responsive:     [
                        {
                        breakpoint: tootoBreakpoints.lg,
                            settings: {
                        ...
                        {
                        breakpoint: tootoBreakpoints.md,
                            settings: {
    ...
    var TimelineHandler = function ( $scope, $ ) {
		...
		if ( isEditMode ) {
			settings.window = tooto.$previewContents;
		}
    ...
    var CardSliderHandler = function ($scope, $) {
		...
		if ( 'no' !== elementSettings.open_lightbox ) {
			$scope.find('.pp-card-slider-item.swiper-slide-duplicate').each(function () {
				...
				lightboxItem.removeAttr( 'data-tooto-open-lightbox data-tooto-lightbox-slideshow data-tooto-lightbox-index' );
				lightboxItem.removeClass( 'tooto-clickable' );
			});
		}
    ...
    $(window).on('tooto/frontend/init', function () {
		...
		$.each( widgets, function( widget, callback ) {
			tootoFrontend.hooks.addAction( 'frontend/element_ready/' + widget, callback );
		});

		tootoFrontend.hooks.addAction( 'frontend/element_ready/global', WrapperLinkHandler );
		tootoFrontend.hooks.addAction( 'frontend/element_ready/widget', ElementsTooltipHandler );
    });
