;( function( tooto, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		ui: {
			insertButton: '.tooto-template-library-template-insert'
		},
		events: {
	...
	var TemplateLibraryTemplateView = Marionette.ItemView.extend({
		className: function className() {
			var classes = 'tooto-template-library-template',
				source = this.model.get('source');
			classes += ' tooto-template-library-template-remote';
			classes += ' tooto-template-library-template-' + source;

			if ('powerpack' === source) {
				classes += ' tooto-template-library-template-' + this.model.get('type');
			}

			if (this.model.get('isPro')) {
				classes += ' tooto-template-library-pro-template';
			}
		...
		ui: function ui() {
			return {
				previewButton: '.tooto-template-library-template-preview'
			};
		},
	...
	var TemplateLibraryTemplatePowerPackView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-tooto-template-library-template-pp',
		ui: function ui() {
			return jQuery.extend(TemplateLibraryTemplateView.prototype.ui.apply(this, arguments), {
				favoriteCheckbox: '.tooto-template-library-template-favorite-input'
			});
		},
		events: function events() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			if (!isFavorite && pp_templates_lib.templates.getFilter('favorite')) {
				tooto.channels.templates.trigger('filter:change');
			}
		}
	});

	var TemplateLibraryPreviewView = Marionette.ItemView.extend( {
		template: '#tmpl-tooto-template-library-preview',
	
		id: 'tooto-template-library-preview',
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-tooto-template-library-header-preview-pp',
		id: 'tooto-template-library-header-preview',
		behaviors: {
			insertTemplate: {
				behaviorClass: TemplateLibraryInsertTemplateBehavior
			}
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		template: '#tmpl-tooto-template-library-header-back',
	
		id: 'tooto-template-library-header-preview-back',
	...
	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-tooto-template-library-templates-pp',
		id: 'tooto-template-library-templates',
		childViewContainer: '#tooto-template-library-templates-container',
		reorderOnSort: true,
		ui: {
			textFilter: '#tooto-template-library-filter-text',
			selectFilter: '.tooto-template-library-filter-select',
			myFavoritesFilter: '#tooto-template-library-filter-my-favorites',
			orderInputs: '.tooto-template-library-order-input',
			orderLabels: 'label.tooto-template-library-order-label'
		},
		events: {
		...
		initialize: function initialize() {
			this.listenTo(tooto.channels.templates, 'filter:change', this._renderChildren);
		},
		...
		setFiltersUI: function setFiltersUI() {
			...
			$filters.select2({
				placeholder: tooto.translate('category'),
				allowClear: true,
				width: 150,
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('tooto-templates-filter-active', !!(pp_templates_lib.templates.getFilter('text') || pp_templates_lib.templates.getFilter('favorite')));
		},
		...
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('tooto-filter');
		...
		onOrderLabelsClick: function onOrderLabelsClick(event) {
			...
			$clickedInput.toggleClass('tooto-template-library-order-reverse', toggle);
			this.order($clickedInput.val(), $clickedInput.hasClass('tooto-template-library-order-reverse'));
		}
	});

	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-tooto-templates-modal__header__logo_pp',
		className: function className() {
			var classes = 'tooto-templates-modal__header__logo';
			return classes;
		},
		events: function events() {
			return {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-tooto-template-library-header-actions-pp',
		id: 'tooto-template-library-header-actions',
		ui: {
			sync: '#tooto-template-library-header-sync i',
		},
		events: {
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		template: '#tmpl-tooto-template-library-header-menu',
		id: 'tooto-template-library-header-menu',
		templateHelpers: function templateHelpers() {
			return {
	...
	var TemplateLibraryLayoutView = tootoModules.common.views.modal.Layout.extend({
		getModalOptions: function() {
			return {
				id: 'tooto-template-library-modal',
				effects: {
					show: 'show',
					hide: 'hide'
				},
		...
		getTemplateActionButton: function(templateData) {
			var viewId = '#tmpl-tooto-template-library-insert-button';
			if ( templateData.isPro && 'inactive' === templateData.proStatus ) {
				viewId = '#tmpl-tooto-template-library-get-pro-button-pp';
			}
			if ( templateData.isPro && 'license_inactive' === templateData.proStatus ) {
				viewId = '#tmpl-tooto-pro-template-library-activate-license-button-pp';
			}
			//viewId = tooto.hooks.applyFilters('tooto/editor/pp-template-library/template/action-button', viewId, templateData);
	...
	var Component = function(ComponentModal) {
				...
				'templates/blocks': {
					...
					getFilter: function() {
						return {
							source: 'powerpack',
							type: 'block',
							subtype: tooto.config.document.remoteLibrary.category,
						}
					}
				},
				'templates/pages': {
		...
		Component.prototype.insertTemplate = function( args ) {
			...
			Component.prototype.getImportSettingsDialog = function() {
				var InsertTemplateHandler = {
					...
					initDialog: function() {
						InsertTemplateHandler.dialog = tootoCommon.dialogsManager.createWidget( 'confirm', {
							id: 'tooto-insert-template-settings-dialog',
							headerMessage: tooto.translate( 'import_template_dialog_header' ),
							message: tooto.translate( 'import_template_dialog_message' ) + '<br>' + tooto.translate( 'import_template_dialog_message_attention' ),
							strings: {
								confirm: tooto.translate( 'yes' ),
								cancel: tooto.translate( 'no' ),
							},
						} );
					},
		...
		Component.prototype.getTabsWrapperSelector = function() {
			return '#tooto-template-library-header-menu';
		};
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.getFilter = function (name) {
			return tooto.channels.templates.request('filter:' + name);
		};

		this.setFilter = function (name, value, silent) {
			tooto.channels.templates.reply('filter:' + name, value);
		...
		this.setScreen = function (args) {
			tooto.channels.templates.stopReplying();
		...
		this.requestLibraryData = function (options) {
			...
			tootoCommon.ajax.addRequest('pp_get_library_data', ajaxOptions);
		};
		...
		this.requestTemplateContent = function (source, id, ajaxOptions) {
			...
			return tootoCommon.ajax.addRequest('get_template_data', options);
		};
		...
		this.markAsFavorite = function (templateModel, favorite) {
			...
			return tootoCommon.ajax.addRequest('mark_template_as_favorite', options);
		};

		this.getErrorDialog = function() {
			if ( ! errorDialog ) {
				errorDialog = tootoCommon.dialogsManager.createWidget( 'alert', {
					id: 'tooto-template-library-error-dialog',
					headerMessage: tooto.translate( 'an_error_occurred' ),
				} );
			}
		...
		this.showErrorDialog = function( errorMessage ) {
			...
			self.getErrorDialog()
				.setMessage( tooto.translate( 'templates_request_error' ) + '<div id="tooto-template-library-error-info">' + errorMessage + '</div>' )
				.show();
		};
	...
	tooto.on('document:loaded', function() {
		var $previewContents = tooto.$previewContents;
	
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.tooto-add-section-area-button.pp-add-template-button { margin-left: 8px; vertical-align: bottom; }';
			btnStyle += '.tooto-add-section-area-button.pp-add-template-button img { height: 40px; }';
		...
		$templateLibBtn.addClass( 'tooto-add-section-area-button pp-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add PowerPack Template' );
		$templateLibBtn.html( '<img src="' + pp_templates_lib.logoUrl + '" />' );
		$templateLibBtn.insertAfter( $previewContents.find('.tooto-add-section-area-button.tooto-add-template-button') );
		$templateLibBtn.on( 'click', function() {
			$e.run('powerpack/open');
		});

		$previewContents.find('body').delegate('.tooto-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				var sectionInner = $previewContents.find('body').find('.tooto-add-section-inner');
				...
				if ( sectionInner.length > 0 ) {
					...
					$templateLibBtn.insertAfter( sectionInner.find('.tooto-add-section-area-button.tooto-add-template-button') );
				}
			}, 100);
		});
	});
