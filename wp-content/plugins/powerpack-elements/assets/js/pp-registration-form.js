(function($) {
						...
						callback: function (response) {
							if ( response != '' ) {
								...
								if ('invisible' == self.data('validate')) {
									self.closest('.tooto-widget').find('.pp-submit-button').trigger('click');
								}
							}
						}
					});
	...
	PPRegistrationForm.prototype = {
		...
		_initReCaptcha: function() {
			...
			if ( tootoFrontend.isEditMode() && undefined == reCaptchaField.attr( 'data-widgetid' ) ) {
				onLoadPPReCaptcha();
			}
		},
		...
		_submit: function(e) {
			...
			var theForm			= this.form,
				...
				nodeId      	= theForm.closest( '.tooto-widget-pp-registration-form' ).data( 'id' );
				//nodeId      	= theForm.parents( '.tooto-element.tooto-widget' ).last().data( 'id' );
			...
			// Validate Required.
			var self = this;
			theForm.find('.pp-rf-field.tooto-mark-required').each(function() {
				var field    = $(this),
