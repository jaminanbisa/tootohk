( function ( $ ) {
	...
	var item_type_tooto_hook = [ 'widget', 'column', 'section', 'container' ];
	...
	PPMWHandler = {
		...
		paste: function( data, element ) {
			...
			switch( data.elType ) {
				...
				case 'container':
					...
					container = tooto.getPreviewContainer();
		...
		createElement: function( targetElement, targetElementType, data ) {
			...
			if ( undefined == element ) {
				if ( "widget" === targetElementType ) {
					if ( targetElement.$el.next( '.undefined.tooto-widget-empty' )  ) {
						targetElement.$el.next( '.undefined.tooto-widget-empty' ).after( '<div class="tooto-alert tooto-alert-warning">' + pp_magic_wand.widget_not_found + '</div>' );
					}
				} else {
					if ( targetElement.$el.find( '.undefined.tooto-widget-empty' ) ) {
						targetElement.$el.find( '.undefined.tooto-widget-empty' ).after( '<div class="tooto-alert tooto-alert-warning">' + pp_magic_wand.widget_not_found + '</div>' );
					}
				}
			}
		...
		generateUniqueID: function( elements ) {		
			elements.forEach( function( item, index ) {
				if ( typeof tootoCommon.helpers.getUniqueId() != "undefined" ) {
					item.id = tootoCommon.helpers.getUniqueId();
				}
	...
	if ( 'undefined' !== typeof tooto ) {

		item_type.forEach( function( item, index ) {
			tooto.hooks.addFilter( 'elements/' + item_type_tooto_hook[index] + '/contextMenuGroups', function ( groups, element ) {
