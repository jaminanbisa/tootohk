(function ($) {
    ...
    var BgHandler = function ($scope, $) {
		...
		var bg_row_elem    = tootoFrontend.isEditMode() ? $scope.find('.pp-background-wrapper') : $scope,
		...
		var prepend_content  = function( canvas_opacity ) {
			if ( ! tootoFrontend.isEditMode() ) {
				...
				if ( $scope.find( '.tooto-background-overlay' ).length == 0 ) {
					bg_row_elem.prepend( content );
				} else {
					content.insertBefore( bg_row_elem.find('.tooto-container') );
				}
			} else {
    ...
    $( window ).on('tooto/frontend/init', function () {
		tootoFrontend.hooks.addAction('frontend/element_ready/global', BgHandler);
    });
