(function ($) {
	...
	var WidgetPPWooProducts = function ($scope, $, $panel) {
		...
		if (slider_wrapper.length > 0) {
			...
			jQuery(carouselWrap).closest('.tooto-widget-wrap').addClass('e-swiper-container');
			jQuery(carouselWrap).closest('.tooto-widget').addClass('e-widget-swiper');
	...
	.on( 'click', '.pp-woocommerce-pagination a.page-numbers', function( e ) {
		...
		$scope = $( this ).closest( '.tooto-widget-pp-woo-products' );
			...
			.on("added_to_cart.pp_cart", function (
				...
				if (
					$(this_button)
						...
						.parent()
						.parent()
						.hasClass("tooto-widget-pp-woo-add-to-cart")
				) {
					$btn = $(this_button);
	...
	var WooCategoriesHandler = function ($scope, $) {
		...
		jQuery(slider_wrapper).closest('.tooto-widget-wrap').addClass('e-swiper-container');
		jQuery(slider_wrapper).closest('.tooto-widget').addClass('e-widget-swiper');
	...
	$(window).on("tooto/frontend/init", function () {
		...
		//tootoFrontend.hooks.addAction('frontend/element_ready/pp-woo-product-tabs.default', WooProductTabsHandler);
	});
})(jQuery);
