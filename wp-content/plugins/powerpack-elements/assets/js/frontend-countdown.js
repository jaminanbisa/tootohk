(function ($) {
		...
		_initFixedTimer: function () {
			...
			if ((dateNow.getTime() - this.timer_date.getTime()) > 0) {
				if (this.fixed_timer_action == 'msg') {
					if (parseInt(window.location.href.toLowerCase().indexOf("?tooto")) === parseInt(-1)) {
	...
	$(window).on('tooto/frontend/init', function () {
		...
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-countdown.default', CountdownHandler );
    });
