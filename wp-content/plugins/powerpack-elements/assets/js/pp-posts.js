( function( $ ) {
	...
	var PostsHandler = function( $scope, $ ) {
		...
		if ( container.find('.pp-posts').hasClass( 'pp-posts-skin-checkerboard' ) && $scope.hasClass( 'pp-equal-height-yes' ) ) {
			function setEqualHeight() {
				...
				if ($(window).width() >= tootoBreakpoints.md) {
					$height = maxHeight;
				}
		...
		if ( 'carousel' == layout ) {
			...
			$(carouselWrap).closest('.tooto-widget-wrap').addClass('e-swiper-container');
			$(carouselWrap).closest('.tooto-widget').addClass('e-widget-swiper');
			...
			if ( carousel.length > 0 ) {
				if ( 'undefined' === typeof Swiper ) {
					var asyncSwiper = tootoFrontend.utils.swiper;
	...
	$( document ).on( 'click', '.pp-post-load-more', function( e ) {
		...
		$scope = $( this ).closest( '.tooto-widget-pp-posts' );
		loader = $scope.find( '.pp-posts-loader' );

		//Coupon widget
		var $coupon_scope = $( this ).closest( '.tooto-widget-pp-coupons' );
	...
	var _callAjax = function( $scope, $obj, $append, $count ) {
		...
		$.ajax({
			...
			if ( $scope.find( '.tooto-invisible' ).length > 0 ) {
				$scope.find( '.tooto-invisible' ).removeClass( 'tooto-invisible' );
			}
		} );
	}
	...
	$( window ).on( 'tooto/frontend/init', function () {
		...
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.classic', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.card', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.checkerboard', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.creative', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.event', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.news', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.portfolio', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.overlap', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-posts.template', PostsHandler );
		
		tootoFrontend.hooks.addAction( 'frontend/element_ready/pp-coupons.default', PostsHandler );
