(function ($) {
	var PPToCHandler = tootoModules.frontend.handlers.Base.extend({
		stretchElement: null,
		...
		getDefaultSettings: function () {
			...
			return {
				selectors: {
					...
					container:
						'.tooto:not([data-tooto-type="header"]):not([data-tooto-type="footer"])',
		...
		bindEvents: function bindEvents() {
			...
			if (elementSettings.sticky_toc_toggle) {
				tootoFrontend.elements.$window.on("resize", this.handleStickyToc);
			}
		...
		populateTOC: function populateTOC() {
			...
			$(window).on('scroll', function(){
				...
				if ("window_top" === elementSettings.scroll_to_top_option) {
					...
					var $id = self.getID().parents('.tooto-widget-wrap');
		...
		collapseOnInit: function collapseOnInit() {
			...
			if ( 0 !== minimizedOn.length  && "object" === typeof minimizedOn ) {
				minimizedOn.forEach(function (value) {
					if (
						( "desktop" === value[0] && "desktop" == currentDeviceMode && $(window).width() < tootoFrontend.config.breakpoints.xxl ) ||
						( "tablet" === value[0] && "tablet" === currentDeviceMode && $(window).width() < tootoFrontend.config.breakpoints.lg ) ||
						( "mobile" === value[0] && "mobile" === currentDeviceMode && $(window).width() < tootoFrontend.config.breakpoints.md )
					) {
						self.collapseBox();
					}
				});
		...
		stickyScroll: function (){
			...
			var target = this.getID();
			var elementSettings = this.getElementSettings();
			var item = document.querySelector(".tooto-widget-pp-table-of-contents");			
		...
		scrollToTop: function scrollToTop() {
			...
			if ("window_top" === scrollTo) {
				...
				var $id = this.getID().parents('.tooto-widget-pp-table-of-contents');
	...
	jQuery(window).on("tooto/frontend/init", function () {
		var addHandler = function ($element) {
			tootoFrontend.elementsHandler.addHandler(PPToCHandler, {
				$element: $element,
			});
		};
		tootoFrontend.hooks.addAction(
