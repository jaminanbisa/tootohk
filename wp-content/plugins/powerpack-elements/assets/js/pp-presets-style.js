( function( $ ) {
	jQuery( window ).on( 'tooto:init', function() {
		...
		var preset_data = tooto.modules.controls.BaseData.extend({
			...
			applyPresets: function ( presets_json ) {
				var e = tooto.getPanelView().getCurrentPageView().getOption( "editedElementView" );
				//Reset style.
		...
		tooto.addControlView( "pp-presets-style", preset_data );
	});
} )( jQuery ); 
