(function( $ ) {
    ...
    var VisualWavesAnimation = {
        initWaves: function () {
            tootoFrontend.hooks.addAction('frontend/element_ready/section', VisualWavesAnimation.initWavesWidget);
        },
    ...
    function wavesAnimation(target,settings,sectionId) {
        ...
        if (checkElement.length >= 0) {
            ...
            var wavesZindex = document.querySelector('.marvy-custom-waves-animation-section-'+sectionId+' .tooto-container');
            wavesZindex.style.zIndex = '99';

            // Set min height
            var wavesMinHeight = document.querySelector(".tooto-element-"+sectionId);
            wavesMinHeight.closest('.tooto-top-section').style.minHeight = "200px";
    ...
    $( window ).on('tooto/frontend/init', VisualWavesAnimation.initWaves);
})( jQuery );
