(function ($) {
    ...
    var VisualRipplesAnimation = {
        initRipples: function () {
            tootoFrontend.hooks.addAction('frontend/element_ready/section', VisualRipplesAnimation.initRipplesWidget);
        },
    ...
    function rippleAnimation(target, settings, sectionId, sectionKey) {
        ...
        if (checkElement.length >= 0) {
            ...
            var ripplesZindex = document.querySelector('.marvy-custom-ripples-animation-section-' + sectionId + ' .tooto-container');
    ...
    function marvyRipplesRender(target, sectionId, sectionKey) {
        ...
        if(myEle.length!==0){
            document.querySelector(".tooto-element-" + sectionId).addEventListener('DOMAttrModified', marvyRippleResizedEvent.bind(event, target, sectionId, sectionKey));
        }
    ...
    function marvyRippleResizedEvent(target, sectionId, sectionKey) {
        var sectionStyle = getStyle(document.querySelector(".tooto-element-" + sectionId));
    ...
    $(window).on('tooto/frontend/init', VisualRipplesAnimation.initRipples);
})(jQuery);
