(function( $ ) {
    ...
    var MarvyBannerAnimation = {
        initBanner: function () {
            tootoFrontend.hooks.addAction('frontend/element_ready/section', MarvyBannerAnimation.initBannerWidget);
        },
    ...
    $( window ).on('tooto/frontend/init', MarvyBannerAnimation.initBanner);
    ...
    function bannerAnimation(target,settings,sectionId, sectionKey) {
        ...
        if (checkElement.length >= 0 ) {
            ...
            var bannerZindex = document.querySelector('.marvy-custom-banner-animation-section-'+sectionId+' .tooto-container');
            bannerZindex.style.zIndex = '99';

            var bannerMinHeight = document.querySelector(".tooto-element-"+sectionId);
            bannerMinHeight.closest('.tooto-top-section').style.minHeight = "200px";
