(function( $ ) {
    ...
    var MarvyColorDropAnimation = {
        initColorDrop: function () {
            tootoFrontend.hooks.addAction('frontend/element_ready/section', MarvyColorDropAnimation.initColorDropWidget);
        },
        initColorDropWidget: function ($scope) {
            let sectionId = $scope.data('id');
            let target = '.tooto-element-'+ sectionId;
            let settings = {};
            if (window.isEditMode || window.tootoFrontend.isEditMode()) {
                ...
                colorDropEditorElements = window.tooto.elements;
    ...
    $( window ).on('tooto/frontend/init', MarvyColorDropAnimation.initColorDrop);
    ...
    function colorDropAnimation(target,settings,sectionId,sectionKey) {
        ...
        if (checkElement.length <= 0 ) {
            ...
            let zIndex = document.querySelector('.marvy-custom-color-drop-section-'+sectionId+' .tooto-container');
            zIndex.style.zIndex = '99';

            let colorDropMinHeight = document.querySelector(".tooto-element-"+sectionId);
            colorDropMinHeight.closest('.tooto-top-section').style.minHeight = "400px";
