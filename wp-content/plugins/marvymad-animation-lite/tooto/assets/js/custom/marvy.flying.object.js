(function( $ ) {
    ...
    var MarvyPureAnimation = {
        initPure: function () {
            tootoFrontend.hooks.addAction('frontend/element_ready/section', MarvyPureAnimation.initRipplesWidget);
        },
    ...
    $( window ).on('tooto/frontend/init', MarvyPureAnimation.initPure);
    ...
    function pureAnimation(target,settings,sectionId) {
        ...
        if (checkElement.length <= 0 ) {
            ...
            var zIndex = document.querySelector('.marvy-custom-pure-animation-section-'+sectionId+' .tooto-container');
            zIndex.style.zIndex = '99';

            var pureMinHeight = document.querySelector(".tooto-element-"+sectionId);
            pureMinHeight.closest('.tooto-top-section').style.minHeight = "100px";
