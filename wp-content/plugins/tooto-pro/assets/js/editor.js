...
module.exports = tootoModules.editor.utils.Module.extend({
  elementType: null,
  ...
  addEditorListener() {
    ...
    if (self.onElementChange) {
      ...
      tooto.channels.editor.on(eventName, function (controlView, elementView) {
  ...
  addControlSpinner(controlName) {
    ...
    if ($el.find('.tooto-control-spinner').length) {
      return;
    }
    const $input = $el.find(':input');
    $input.attr('disabled', true);
    $el.find('.tooto-control-title').after('<span class="tooto-control-spinner"><i class="eicon-spinner eicon-animation-spin"></i>&nbsp;</span>');
  },
  /**
  ...
  removeControlSpinner(controlName) {
    ...
    $controlEl.find('.tooto-control-spinner').remove();
  },
  /**
  ...
  addControlError(controlName, error) {
    let location = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '.tooto-control-content';
    const $el = this.getEditorControlView(controlName).$el;
    ...
    $el.find(location).first().after(`<span class="tooto-control-field-description e-control-error">${error}</span>`);
  },
  /**
...
module.exports = tootoModules.editor.views.ControlsStack.extend({
  activeTab: 'content',
...
const EDIT_HANDLE_CLASS_NAME = 'tooto-document-handle';
const EDIT_MODE_CLASS_NAME = 'tooto-edit-mode';
const EDIT_CONTEXT = 'edit';
exports.EDIT_CONTEXT = EDIT_CONTEXT;
const SAVE_HANDLE_CLASS_NAME = 'tooto-document-save-back-handle';
...
function addDocumentHandle(_ref) {
  ...
  if (EDIT_CONTEXT === context) {
    element.dataset.editabletootoDocument = id;
  }
}
...
function createHandleElement(_ref2, context) {
  ...
  const element = createElement({
    ...
    children: [createElement({
      ...
      children: [document.createTextNode(EDIT_CONTEXT === context ? __('Edit %s', 'tooto-pro').replace('%s', title) : __('Save %s', 'tooto-pro').replace('%s', title))]
    })]
  });
...
function getHandleIcon(context) {
  ...
  if (SAVE_CONTEXT === context) {
    icon = tootoFrontend.config.is_rtl ? 'eicon-arrow-right' : 'eicon-arrow-left';
  }
  return icon;
}
...
async function onDocumentClick(id, context) {
  ...
  if (EDIT_CONTEXT === context) {
    ...
    tootoCommon.api.internal('panel/state-loading');
    tootoCommon.api.run('editor/documents/switch', {
      id: tooto.config.initial_document.id,
      mode: 'save',
      shouldScroll: false,
      selector
    }).finally(() => tootoCommon.api.internal('panel/state-ready'));
  }
}
...
class Module extends tootoModules.editor.utils.Module {
  ontootoInit() {
...
module.exports = tootoModules.Module.extend({
  _enqueuedFonts: [],
  ...
  getCustomFont(fontType, font) {
    tootoPro.ajax.addRequest('assets_manager_panel_action_data', {
      ...
      success(data) {
        if (data.font_face) {
          tooto.$previewContents.find('style').last().after('<style type="text/css">' + data.font_face + '</style>');
        }
        if (data.font_url) {
          tooto.$previewContents.find('link').last().after('<link href="' + data.font_url + '" rel="stylesheet" type="text/css">');
        }
      }
    });
  ...
  onInit() {
    tooto.channels.editor.on('font:insertion', this.onFontChange.bind(this));
  }
});
...
exports["default"] = void 0;
class _default extends tootoModules.editor.utils.Module {
  addCustomCss(css, context) {
    ...
    let selector = '.tooto-element.tooto-element-' + model.get('id');
    if ('document' === model.get('elType')) {
      selector = tooto.config.document.settings.cssWrapperSelector;
    }
    if (customCSS) {
  ...
  ontootoInit() {
    tooto.hooks.addFilter('editor/style/styleText', this.addCustomCss);
    tooto.on('navigator:init', this.onNavigatorInit.bind(this));
  }
  onNavigatorInit() {
    tooto.navigator.indicators.customCSS = {
      icon: 'code-bold',
      settingKeys: ['custom_css'],
      title: __('Custom CSS', 'tooto-pro'),
...
module.exports = tootoModules.editor.utils.Module.extend({
  ontootoInit() {
    tooto.channels.editor.on('section:activated', this.onSectionActivated);
  },
  onSectionActivated(sectionName, editor) {
    ...
    editedElement.$el.toggleClass('tooto-flip-box--flipped', isSideBSection);
    var $backLayer = editedElement.$el.find('.tooto-flip-box__back');
    if (isSideBSection) {
...
module.exports = tooto.modules.controls.Repeater.extend({
  onBeforeRender() {
    this.$el.hide();
  },
  updateMap(fields) {
  ...
  onRender() {
    tooto.modules.controls.Base.prototype.onRender.apply(this, arguments);
    var self = this;
    self.children.each(function (view) {
      ...
      if (view.model.get('remote_required')) {
        label += '<span class="tooto-required">*</span>';
      }
      ...
      view.$el.find('.tooto-repeater-row-tools').hide();
      view.$el.find('.tooto-repeater-row-controls').removeClass('tooto-repeater-row-controls').find('.tooto-control').css({
        paddingBottom: 0
      });
    });
    self.$el.find('.tooto-button-wrapper').remove();
...
module.exports = class extends tooto.modules.controls.Repeater {
  className() {
    let classes = super.className();
    classes += ' tooto-control-type-repeater';
    return classes;
  }
  getChildView() {
...
exports["default"] = void 0;
class _default extends tooto.modules.controls.RepeaterRow {
  ...
  toggleStepField(isStep) {
    this.$el.toggleClass('tooto-repeater-row--form-step', isStep);
  }
  toggleTools(show) {
...
module.exports = tootoModules.editor.utils.Module.extend({
  renderField(inputField, item, i, settings) {
    ...
    return '<div class="tooto-field-subgroup">' + '<span class="tooto-field-option">' + '<input size="1" type="checkbox"' + checked + ' class="tooto-acceptance-field tooto-field tooto-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' > ' + label + '</span></div>';
  },
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/acceptance', this.renderField, 10, 4);
  }
});
...
module.exports = tootoModules.editor.utils.Module.extend({
  renderField(inputField, item, i, settings) {
    ...
    if ('yes' === item.use_native_date) {
      itemClasses += ' tooto-use-native';
    }
    return '<input size="1"' + min + max + placeholder + ' pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}" type="date" class="tooto-field-textual tooto-date-field tooto-field tooto-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' >';
  },
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/date', this.renderField, 10, 4);
  }
});
...
module.exports = tootoModules.editor.utils.Module.extend({
  renderField(inputField, item, i, settings) {
    ...
    itemClasses = 'tooto-field-textual ' + itemClasses;
    return '<input size="1" type="' + item.field_type + '" class="tooto-field-textual tooto-field tooto-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' ' + placeholder + ' pattern="[0-9()-]" >';
  },
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/tel', this.renderField, 10, 4);
  }
});
...
module.exports = tootoModules.editor.utils.Module.extend({
  renderField(inputField, item, i, settings) {
    ...
    return '<input size="1" type="time"' + placeholder + ' class="tooto-field-textual tooto-time-field tooto-field tooto-size-' + settings.input_size + ' ' + itemClasses + '" name="form_field_' + i + '" id="form_field_' + i + '" ' + required + ' >';
  },
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/time', this.renderField, 10, 4);
  }
});
...
module.exports = tootoModules.editor.utils.Module.extend({
  renderField(inputField, item, i, settings) {
    ...
    return '<input size="1"  type="file" class="tooto-file-field tooto-field tooto-size-' + settings.input_size + ' ' + itemClasses + '" name="' + fieldName + '" id="form_field_' + i + '" ' + required + multiple + ' >';
  },
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/upload', this.renderField, 10, 4);
  }
});
...
class FormFieldsSanitizeCustomId extends $e.modules.hookData.Dependency {
  ...
  getId() {
    return 'tooto-pro-forms-fields-sanitize-custom-id';
  }
  getContainerType() {
    return 'repeater';
  }
...
class FormFieldsSetCustomId extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-forms-fields-set-custom-id';
  }
  getContainerType() {
    return 'widget';
  }
...
class FormFieldsAddFirstStep extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-forms-fields-first-step';
  }
  getContainerType() {
    return 'repeater';
  }
...
class FormSanitizeId extends $e.modules.hookData.Dependency {
  ...
  getId() {
    return 'tooto-pro-forms-sanitize-id';
  }
  getContainerType() {
    return 'widget';
  }
...
class FormFieldsUpdateShortCode extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-forms-fields-update-shortcode';
  }
  getContainerType() {
    return 'repeater';
  }
...
module.exports = BaseIntegrationModule.extend({
  ...
  updateFieldsMapping() {
    ...
    var remoteFields = [{
      remote_label: __('Email', 'tooto'),
      remote_type: 'email',
      remote_id: 'email',
      remote_required: true
    }, {
      remote_label: __('First Name', 'tooto'),
      remote_type: 'text',
      remote_id: 'first_name',
      remote_required: false
    }, {
      remote_label: __('Last Name', 'tooto'),
      remote_type: 'text',
      remote_id: 'last_name',
      remote_required: false
    }, {
      remote_label: __('Phone', 'tooto'),
      remote_type: 'text',
      remote_id: 'phone',
      remote_required: false
    }, {
      remote_label: __('Organization name', 'tooto'),
      remote_type: 'text',
      remote_id: 'orgname',
      remote_required: false
    }];
...
var ElementEditorModule = __webpack_require__(/*! tooto-pro/editor/element-editor-module */ "../assets/dev/js/editor/element-editor-module.js");
module.exports = ElementEditorModule.extend({
  ...
  fetchCache(type, cacheKey, requestArgs) {
    ...
    return tootoPro.ajax.addRequest('forms_panel_action_data', {
...
module.exports = BaseIntegrationModule.extend({
  ...
  updateFieldsMapping() {
    ...
    const remoteFields = [{
      ...
      remote_label: __('Country', 'tooto'),
      remote_type: 'text',
      remote_id: 'country',
      remote_required: false
    }, {
      remote_label: __('State', 'tooto'),
      remote_type: 'text',
      remote_id: 'state',
      remote_required: false
    }, {
      remote_label: __('City', 'tooto'),
      remote_type: 'text',
      remote_id: 'city',
      remote_required: false
    }, {
      remote_label: __('Zip', 'tooto'),
      remote_type: 'text',
      remote_id: 'zip',
      remote_required: false
    }];
...
class FormsModule extends tootoModules.editor.utils.Module {
  ontootoInit() {
    ...
    tooto.addControlView('Fields_map', __webpack_require__(/*! ./fields-map-control */ "../modules/forms/assets/js/editor/fields-map-control.js"));
    tooto.addControlView('form-fields-repeater', __webpack_require__(/*! ./fields-repeater-control */ "../modules/forms/assets/js/editor/fields-repeater-control.js"));
  }
  ontootoInitComponents() {
    $e.components.register(new _component.default({
      manager: this
    }));
  }
...
module.exports = tootoModules.editor.utils.Module.extend({
  enqueueRecaptchaJs(url, type) {
    if (!tootoFrontend.elements.$body.find('[src="' + url + '"]').length) {
      tootoFrontend.elements.$body.append('<scr' + 'ipt src="' + url + '" id="recaptcha-' + type + '"</scri' + 'pt>');
    }
  },
  renderField(inputField, item) {
    inputField += '<div class="tooto-field ' + item.field_type + ' ">';
    inputField += this.getDataSettings(item);
    inputField += '</div>';
    return inputField;
  },
  getDataSettings(item) {
    const config = tootoPro.config.forms[item.field_type],
      srcURL = 'https://www.google.com/recaptcha/api.js?render=explicit';
    if (!config.enabled) {
      return '<div class="tooto-alert tooto-alert-info">' + config.setup_message + '</div>';
    }
    ...
    return '<div class="tooto-g-recaptcha' + _.escape(item.css_classes) + '" ' + recaptchaData + '></div>';
  },
  filterItem(item) {
  ...
  onInit() {
    tooto.hooks.addFilter('tooto_pro/forms/content_template/item', this.filterItem);
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/recaptcha', this.renderField, 10, 2);
    tooto.hooks.addFilter('tooto_pro/forms/content_template/field/recaptcha_v3', this.renderField, 10, 2);
  }
});
...
module.exports = function () {
  ...
  var updateReplyToOptions = function () {
    ...
    emailFields = _.map(emailModels, function (model) {
      return {
        ...
        label: sprintf(__('%s Field', 'tooto-pro'), model.get('field_label'))
      };
    });
  ...
  var init = function () {
    tooto.hooks.addAction('panel/open_editor/widget/form', onPanelShow);
  };
  init();
};
...
class SaveTemplates extends $e.modules.CommandInternalBase {
  apply() {
    ...
    return new Promise((resolve, reject) => {
      tootoCommon.ajax.addRequest('update_templates', {
        data: {
  ...
  getCurrentTemplatesModels(changedContainersId) {
    ...
    Object.entries(changedContainersId).forEach(_ref => {
      ...
      const container = tooto.getContainer(containerId);
      if (!container) {
        return;
      }
...
class Link extends $e.modules.document.CommandHistory {
  ...
  getHistory(args) {
    ...
    return {
      title: tooto.widgetsCache[data.widgetType].title,
      subTitle: data.title,
      type: __('Linked to Global', 'tooto-pro')
    };
  }
  apply(args) {
    const {
    ...
    containers.forEach(( /** Container */container) => {
      ...
      const elementModel = tootoPro.modules.globalWidget.addGlobalWidget(data.template_id, data),
...
class Unlink extends $e.modules.document.CommandHistory {
  ...
  getHistory(args) {
    ...
    return {
      title: tooto.helpers.getModelLabel(containers[0].model),
      // TODO: add support multi containers.
      type: __('Unlink Widget', 'tooto-pro')
    };
  }
  async apply(args) {
    const {
    ...
    containers.forEach(( /** Container */container) => {
      const id = container.model.get('templateID'),
        elementModel = tootoPro.modules.globalWidget.createGlobalModel(id, data[id]);
      $e.run('document/elements/create', {
        container: container.parent,
        model: {
          id: tooto.helpers.getUniqueID(),
          ...
          settings: tootoCommon.helpers.cloneObject(elementModel.get('settings').attributes),
          defaultEditSettings: tootoCommon.helpers.cloneObject(elementModel.get('editSettings').attributes)
        },
        options: {
...
class Component extends $e.modules.ComponentBase {
  ...
  updateGlobalsRecursive(targetContainer) {
    ...
    tooto.getPreviewContainer().forEachChildrenRecursive(container => {
      // Will skip self.
...
class GlobalWidgetPrepareUpdateElementSetSettings extends _baseGlobalWidgetPrepareUpdate.BaseGlobalWidgetPrepareUpdate {
  ...
  getId() {
    return 'tooto-pro-global-widget-prepare-update-element-set-settings';
  }
}
...
class GlobalWidgetDoUpdate extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-global-widget-do-update';
  }
  getConditions() {
...
class GlobalWidgetPrepareUpdateRepeaterInsert extends _baseGlobalWidgetPrepareUpdate.default {
  ...
  getId() {
    return 'tooto-pro-global-widget-prepare-update-repeater-insert';
  }
}
...
class GlobalWidgetPrepareUpdateRepeaterRemove extends _baseGlobalWidgetPrepareUpdate.BaseGlobalWidgetPrepareUpdate {
  ...
  getId() {
    return 'tooto-pro-global-widget-prepare-update-repeater-remove';
  }
}
...
class GlobalWidgetSaveTemplates extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-global-widget-save-templates';
  }
  getConditions(args) {
    ...
    const {
      document = tooto.documents.getCurrent()
    } = args;
...
class GlobalWidgetLoadTemplates extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-global-widget-load-templates';
  }
  getConditions() {
  ...
  apply() {
    ...
    Object.entries(tootoPro.config.widget_templates).forEach(_ref => {
      let [id, data] = _ref;
      tootoPro.modules.globalWidget.addGlobalWidget(id, data);
      this.addTemplateToCache(id);
    });
  }
  addTemplateToCache(id) {
    const container = tooto.getPreviewContainer().findChildrenRecursive(i => parseInt(i.model.get('templateID')) === parseInt(id));
    if (!container) {
...
class GlobalWidgetHistoryUpdate extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-global-widget-history-update';
  }
  getContainerType() {
    return 'widget';
  }
  getConditions(args) {
    ...
    return !tooto.documents.getCurrent().history.getActive() && containers.some(container => container.model.get('templateID'));
  }
  apply(args) {
    const {
      containers = [args.container]
...
class Module extends tootoModules.editor.utils.Module {
  ...
  createGlobalModel(templateId, templateData) {
    templateData = Object.assign({}, templateData, {
      id: templateId,
      categories: [],
      icon: tooto.widgetsCache[templateData.widgetType].icon,
      widgetType: templateData.widgetType,
      custom: {
        templateID: templateId
      }
    });
    const elementModel = new tooto.modules.elements.models.Element(templateData);
    elementModel.set('id', templateId);
    return elementModel;
  }
  setWidgetType() {
    tooto.hooks.addFilter('element/view', function (DefaultView, model) {
    ...
    tooto.hooks.addFilter('element/model', function (DefaultModel, attrs) {
      if (attrs.templateID) {
  ...
  registerTemplateType() {
    tooto.templates.registerTemplateType('widget', {
      showInLibrary: false,
      saveDialog: {
        title: __('Save your widget as a global widget', 'tooto-pro'),
        description: __('You\'ll be able to add this global widget to multiple areas on your site, and edit it from one single place.', 'tooto-pro')
      },
  ...
  addPanelPage() {
    tooto.getPanelView().addPage('globalWidget', {
      view: __webpack_require__(/*! ./views/panel-page */ "../modules/global-widget/assets/js/editor/views/panel-page.js")
    });
  }
  getGlobalModels(id) {
    tootoCommon.helpers.softDeprecated('tootoPro.modules.globalWidget.getGlobalModels( id )', '3.5.0', '$e.data.getCache( `document/global/global-widget/templates/${ id }` )');
    return $e.data.getCache(this.component, `document/global/global-widget/templates/${id}`);
  }
  saveTemplates() {
    tootoCommon.helpers.softDeprecated('tootoPro.modules.globalWidget.saveTemplates()', '3.5.0', "$e.internal( 'document/global/save-templates' )");
    $e.internal('document/global/save-templates');
  }
  requestGlobalModelSettings(globalModel, callback) {
    tootoCommon.helpers.softDeprecated('tootoPro.modules.globalWidget.requestGlobalModelSettings()', '3.5.0', "$e.data.get( 'document/global/templates' )");
    $e.data.get('document/global/templates', {
      ids: globalModel.id
    }).then(data => {
      callback(data);
  ...
  setWidgetContextMenuSaveAction() {
    tooto.hooks.addFilter('elements/widget/contextMenuGroups', (groups, widget) => {
  ...
  ontootoInit() {
    tooto.on('panel:init', () => {
      tooto.hooks.addFilter('panel/elements/regionViews', regionViews => {
  ...
  ontootoPreviewLoaded(isFirst) {
    if (!isFirst) {
      return;
    }
    this.addPanelPage();
    $e.routes.register('panel/editor', 'global', args => {
      tooto.getPanelView().setPage('globalWidget', 'Global Editing', {
        editedView: args.view
      });
    });
  }
  onWidgetTemplateSaved(data) {
    tooto.templates.layout.hideModal();
    const container = tooto.getContainer(tooto.templates.layout.modalContent.currentView.model.id);
...
module.exports = tooto.modules.layouts.panel.pages.elements.views.Elements.extend({
  id: 'tooto-global-templates',
  getEmptyView() {
...
var GlobalWidgetsView = tooto.modules.layouts.panel.pages.elements.views.Global;
module.exports = GlobalWidgetsView.extend({
  template: '#tmpl-tooto-panel-global-widget-no-templates',
  id: 'tooto-panel-global-widget-no-templates',
  className: 'tooto-nerd-box tooto-panel-nerd-box e-responsive-panel-stretch'
});
...
module.exports = Marionette.ItemView.extend({
  id: 'tooto-panel-global-widget',
  template: '#tmpl-tooto-panel-global-widget',
  ui: {
    editButton: '#tooto-global-widget-locked-edit .tooto-button',
    unlinkButton: '#tooto-global-widget-locked-unlink .tooto-button',
    loading: '#tooto-global-widget-loading'
  },
  events: {
  ...
  buildUnlinkDialog() {
    ...
    return tootoCommon.dialogsManager.createWidget('confirm', {
      id: 'tooto-global-widget-unlink-dialog',
      headerMessage: __('Unlink Widget', 'tooto-pro'),
      message: __('This will make the widget stop being global. It\'ll be reverted into being just a regular widget.', 'tooto-pro'),
      position: {
        my: 'center center',
        at: 'center center'
      },
      strings: {
        confirm: __('Unlink', 'tooto-pro'),
        cancel: __('Cancel', 'tooto-pro')
      },
      onConfirm() {
...
exports["default"] = void 0;
const ElementModel = tooto.modules.elements.models.Element;
class Model extends ElementModel {
  ...
  initSettingsFromTemplate() {
    const id = this.get('templateID'),
      ...
      elementModel = tootoPro.modules.globalWidget.createGlobalModel(id, data);
    this.set('settings', elementModel.get('settings'));
    tootoFrontend.config.elements.data[this.cid] = this.get('settings');
  }
}
...
exports["default"] = void 0;
const WidgetView = tooto.modules.elements.views.Widget;
class View extends WidgetView {
  className() {
    return super.className() + ' tooto-global-widget tooto-global-' + this.model.get('templateID');
  }
  ...
  getContainer() {
    ...
    container.label = container.label + ' (' + __('global', 'tooto-pro') + ')';
    return container;
  }
  render() {
    super.render();
  ...
  removeInlineAddingAttributes() {
    const globalWidgetElementDom = this.el.querySelector('.tooto-inline-editing');
    if (globalWidgetElementDom) {
      globalWidgetElementDom.classList.remove('tooto-inline-editing');
    }
  }
}
...
module.exports = function () {
  ...
  self.onTemplateIdChange = function () {
    var templateID = self.templateIdView.elementSettingsModel.get('template_id'),
      $editButton = self.templateIdView.$el.find('.tooto-edit-template');
    if (!templateID) {
      $editButton.remove();
      return;
    }
    var editUrl = tootoConfig.home_url + '?p=' + templateID + '&tooto';
    if ($editButton.length) {
      $editButton.prop('href', editUrl);
    } else {
      $editButton = jQuery('<a />', {
        target: '_blank',
        class: 'tooto-button tooto-button-default tooto-edit-template',
        href: editUrl,
        html: '<i class="eicon-pencil" /> ' + __('Edit Template', 'tooto-pro')
      });
      self.templateIdView.$el.find('.tooto-control-input-wrapper').after($editButton);
    }
  };
  self.init = function () {
    tooto.hooks.addAction('panel/open_editor/widget/template', self.onPanelShow);
  };
  self.init();
};
...
var _documentHandle = _interopRequireWildcard(__webpack_require__(/*! tooto-pro/preview/utils/document-handle */ "../assets/dev/js/preview/utils/document-handle.js"));
function _getRequireWildcardCache(nodeInterop) { if (typeof WeakMap !== "function") return null; var cacheBabelInterop = new WeakMap(); var cacheNodeInterop = new WeakMap(); return (_getRequireWildcardCache = function (nodeInterop) { return nodeInterop ? cacheNodeInterop : cacheBabelInterop; })(nodeInterop); }
function _interopRequireWildcard(obj, nodeInterop) { if (!nodeInterop && obj && obj.__esModule) { return obj; } if (obj === null || typeof obj !== "object" && typeof obj !== "function") { return { default: obj }; } var cache = _getRequireWildcardCache(nodeInterop); if (cache && cache.has(obj)) { return cache.get(obj); } var newObj = {}; var hasPropertyDescriptor = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var key in obj) { if (key !== "default" && Object.prototype.hasOwnProperty.call(obj, key)) { var desc = hasPropertyDescriptor ? Object.getOwnPropertyDescriptor(obj, key) : null; if (desc && (desc.get || desc.set)) { Object.defineProperty(newObj, key, desc); } else { newObj[key] = obj[key]; } } } newObj.default = obj; if (cache) { cache.set(obj, newObj); } return newObj; }
class loopBuilderModule extends tootoModules.editor.utils.Module {
  ontootoFrontendInit() {
    tootoFrontend.elements.$body.on('click', '.e-loop-empty-view__box-cta', () => {
      this.createTemplate();
    });
    this.createDocumentSaveHandles();
    tooto.on('document:loaded', this.createDocumentSaveHandles.bind(this));
  }
  createTemplate() {
    setTimeout(() => {
      tooto.getPanelView().getCurrentPageView().activateSection('section_layout')._renderChildren();
      this.getEditorControlView('template_id').createTemplate();
    });
  }
  createDocumentSaveHandles() {
    var _tootoFrontend$co, _tootoFrontend$co2;
    Object.entries((_tootoFrontend$co = tootoFrontend.config) === null || _tootoFrontend$co === void 0 ? void 0 : (_tootoFrontend$co2 = _tootoFrontend$co.elements) === null || _tootoFrontend$co2 === void 0 ? void 0 : _tootoFrontend$co2.data).forEach(_ref => {
      let [cid, element] = _ref;
      const elementData = tooto.getElementData(element);
      if (!(elementData !== null && elementData !== void 0 && elementData.is_loop)) {
        return;
      }
      const md = tootoFrontend.elements.$body.find('[data-model-cid="' + cid + '"]');
      if (!md.length) {
        return;
      }
      const masterItem = md.find('[data-tooto-type="loop-item"]').first();
      if (masterItem) {
        (0, _documentHandle.default)({
          element: masterItem,
          id: 0,
          title: '& Back'
        }, _documentHandle.SAVE_CONTEXT, null, '.tooto-' + tooto.config.initial_document.id);
      }
    });
  }
  ontootoLoaded() {
    tooto.on('document:loaded', this.onDocumentLoaded.bind(this));
    tooto.on('document:unload', this.onDocumentUnloaded.bind(this));
  ...
  onDocumentLoaded(document) {
    ...
    tooto.channels.editor.on('tootoLoopBuilder:ApplySourceChange', this.onApplySourceChange);
  }
  onDocumentUnloaded(document) {
    if (!document.config.theme_builder) {
      return;
    }
    tooto.channels.editor.off('tootoLoopBuilder:ApplySourceChange', this.onApplySourceChange);
  }
  onApplySourceChange() {
  ...
  getCtaStyles() {
    ...
    ctaStyle.setAttribute('href', `${tootoAppProConfig.baseUrl}/assets/css/loop-grid-cta.css`);
    return ctaStyle;
  }
  getCtaContent(widgetName) {
    const ctaContent = document.createElement('div');
    ctaContent.classList.add('e-loop-empty-view__container', 'tooto-grid', widgetName);
...
class _default extends tootoModules.editor.utils.Module {
  ...
  onNavigatorInit() {
    tooto.navigator.indicators.motionFX = {
      icon: 'flash',
      title: __('Motion Effects', 'tooto-pro'),
...
class StripeModule extends tootoModules.editor.utils.Module {
  ontootoInit() {
...
const ElementEditorModule = __webpack_require__(/*! tooto-pro/editor/element-editor-module */ "../assets/dev/js/editor/element-editor-module.js");
module.exports = ElementEditorModule.extend({
  __construct() {
    ElementEditorModule.prototype.__construct.apply(this, arguments);
  },
  ...
  onInit() {
    tooto.channels.editor.on('editor:widget:stripe-button:section_stripe_account:activated', this.onSectionActive);
  },
  onSectionActive() {
    return tootoPro.ajax.addRequest('get_stripe_tax_rates', {
      success: data => {
...
exports["default"] = void 0;
class _default extends tootoModules.editor.views.ControlsStack {
  constructor() {
    super(...arguments);
  ...
  className() {
    return super.className() + ' tooto-popup__display-settings';
  }
  toggleGroup(groupName, $groupElement) {
    $groupElement.toggleClass('tooto-active', !!this.model.get(groupName));
  }
  onRenderTemplate() {
  ...
  onRender() {
    ...
    this.children.each(child => {
      ...
      $groupWrapper = jQuery('<div>', {
        id: `tooto-popup__${name}-controls-group--${groupName}`,
        class: 'tooto-popup__display-settings_controls_group'
      });
      const $imageWrapper = jQuery('<div>', {
          class: 'tooto-popup__display-settings_controls_group__icon'
        }),
        $image = jQuery('<img>', {
          src: tootoPro.config.urls.modules + `popup/assets/images/${name}/${groupName}.svg`
        });
...
class PopupSave extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-popup-save';
  }
  getConditions() {
    return 'popup' === tooto.config.document.type;
  }
  apply() {
    const settings = {};
    jQuery.each(tootoPro.modules.popup.displaySettingsTypes, (type, data) => {
      settings[type] = data.model.toJSON({
        remove: ['default']
      });
    });
    tootoPro.ajax.addRequest('popup_save_display_settings', {
      data: {
        settings
      }
    });
...
class PopupRemoveInstructions extends _baseHookPopupAfter.default {
  ...
  getId() {
    return 'tooto-pro-popup-remove-instructions';
  }
  getConditions(args) {
    const {
      document
    } = args;
    return 'popup' === document.config.type && !tooto.config.user.introduction.popupSettings;
  }
  apply() {
...
class PopupRemoveLibraryTab extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-popup-remove-library-tab';
  }
  getConditions(args) {
    const {
      document
...
class PopupRemoveTriggers extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-popup-remove-triggers';
  }
  getConditions(args) {
    const {
      document
  ...
  removePanelFooterSubmenuItems() {
    const displaySettingsTypes = tootoPro.modules.popup.displaySettingsTypes;
    jQuery.each(displaySettingsTypes, type => {
      tooto.getPanelView().footer.currentView.removeSubMenuItem('saver-options', {
        name: type
      });
    });
  }
  removePublishTabs() {
    const component = $e.components.get('theme-builder-publish'),
      displaySettingsTypes = tootoPro.modules.popup.displaySettingsTypes;
    jQuery.each(displaySettingsTypes, type => {
...
class PopupAddInstructions extends _baseHookPopupAfter.default {
  ...
  getId() {
    return 'tooto-pro-popup-add-instructions';
  }
  getConditions(args) {
    const document = tooto.documents.get(args.id);
    return 'popup' === document.config.type && !tooto.config.user.introduction.popupSettings;
  }
  apply() {
  ...
  onPageSettingsClose() {
    ...
    introduction.show(tooto.getPanelView().footer.currentView.ui.settings[0]);
    introduction.setViewed();
    $e.components.get('panel/page-settings').off('route/close', this.component.onPageSettingsCloseHandler);
  }
  getIntroduction() {
    return new tootoModules.editor.utils.Introduction({
      introductionKey: 'popupSettings',
      dialogOptions: {
        id: 'tooto-popup-settings-introduction',
        headerMessage: '<i class="eicon-info"></i>' + __('Please Note', 'tooto-pro'),
        message: __('Popup settings are accessed via the settings icon in the bottom menu', 'tooto-pro'),
        closeButton: true,
...
class PopupAddLibraryTab extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-popup-add-library-tab';
  }
  getConditions(args) {
    const document = tooto.documents.get(args.id);
    return 'popup' === document.config.type;
...
class PopupAddTriggers extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-popup-add-triggers';
  }
  getConditions(args) {
    const document = tooto.documents.get(args.id);
    return 'popup' === document.config.type;
  ...
  apply() {
    if (tooto.panel) {
      ...
      tooto.on('preview:loaded', this.addUI.bind(this));
    }
  }
  addUI() {
  ...
  addPublishTabs() {
    const config = tooto.config.document.displaySettings,
      component = $e.components.get('theme-builder-publish'),
      module = tootoPro.modules.popup;
    jQuery.each(module.displaySettingsTypes, (type, data) => {
      // Init models for editor save.
      data.model = new tootoModules.editor.elements.models.BaseSettings(config[type].settings, {
      ...
      component.addTab(type, {
        ...
        viewOptions: {
          name: type,
          id: `tooto-popup-${type}__controls`,
          model: data.model,
        ...
        image: tootoPro.config.urls.modules + `popup/assets/images/${type}-tab.svg`
      });
    });
  }
  addPanelFooterSubmenuItems() {
    ...
    jQuery.each(displaySettingsTypes, (type, data) => {
      tooto.getPanelView().footer.currentView.addSubMenuItem('saver-options', {
...
class PopupModule extends tootoModules.editor.utils.Module {
  constructor() {
    super(...arguments);
    this.displaySettingsTypes = {
      triggers: {
        icon: 'eicon-click',
        title: __('Triggers', 'tooto-pro'),
        publishScreenDescription: __('What action the user needs to do for the popup to open.', 'tooto-pro')
      },
      timing: {
        icon: 'eicon-cog',
        title: __('Advanced Rules', 'tooto-pro'),
        publishScreenDescription: __('Requirements that have to be met for the popup to open.', 'tooto-pro')
      }
    };
  }
  ontootoLoaded() {
...
module.exports = tootoModules.editor.utils.Module.extend({
  ontootoPreviewLoaded() {
    tooto.addControlView('Query', __webpack_require__(/*! ./editor/query-control */ "../modules/query-control/assets/js/editor/query-control.js"));
    __webpack_require__.e(/*! import() */ "modules_query-control_assets_js_editor_template-query-control_js").then(__webpack_require__.bind(__webpack_require__, /*! ./editor/template-query-control */ "../modules/query-control/assets/js/editor/template-query-control.js")).then(_ref => {
      ...
      return tooto.addControlView('template_query', TemplateQueryControl);
    });
  }
});
...
module.exports = tooto.modules.controls.Select2.extend({
  cache: null,
  isTitlesReceived: false,
  getSelect2Placeholder() {
    return {
      id: '',
      text: __('All', 'tooto-pro')
    };
  },
  ...
  getQueryData() {
    ...
    const autocomplete = tootoCommon.helpers.cloneObject(this.model.get('autocomplete'));
    ...
    return jQuery.extend(tooto.modules.controls.Select2.prototype.getSelect2DefaultOptions.apply(this, arguments), {
      ajax: {
        transport(params, success, failure) {
          ...
          return tootoPro.ajax.addRequest(action, {
            data,
            success,
            error: failure
          });
  ...
  addControlSpinner() {
    ...
    this.$el.find('.tooto-control-title').after('<span class="tooto-control-spinner">&nbsp;<i class="eicon-spinner eicon-animation-spin"></i>&nbsp;</span>');
  },
  onReady() {
...
class DeleteScreenshot extends $e.modules.hookData.After {
  ...
  getConditions(args) {
    const {
      ...
      config = tooto.documents.getCurrent().config;
  ...
  apply() {
    const postId = tooto.documents.getCurrent().id;
    return tootoCommon.ajax.addRequest('screenshot_delete', {
...
class FocusPreview extends $e.modules.hookData.After {
  ...
  apply() {
    setTimeout(() => {
      tooto.$preview[0].contentWindow.scrollBy(0, 0);
    }, 100);
  }
}
...
module.exports = tootoModules.editor.utils.Module.extend({
  config: tootoPro.config.shareButtonsNetworks,
  ...
  getNetworkClass(networkName) {
    ...
    if (tooto.config.icons_update_needed) {
...
module.exports = tooto.modules.controls.RepeaterRow.extend({
  template: '#tmpl-tooto-theme-builder-conditions-repeater-row',
  childViewContainer: '.tooto-theme-builder-conditions-repeater-row-controls',
  conflictCheckedOnFirstRender: false,
  id() {
    return 'tooto-condition-id-' + this.model.get('_id');
  },
  onBeforeRender() {
  ...
  initialize() {
    tooto.modules.controls.RepeaterRow.prototype.initialize.apply(this, arguments);
    this.config = tooto.config.document.theme_builder;
  },
  updateOptions() {
  ...
  setTypeAttribute() {
    ...
    typeView.$el.attr('data-tooto-condition-type', typeView.getControlValue());
  },
  // Moved from `modules/theme-builder/assets/js/editor/conditions/repeater.js`.
  checkConflicts() {
    var modelId = this.model.get('_id'),
      rowId = 'tooto-condition-id-' + modelId,
      errorMessageId = 'tooto-conditions-conflict-message-' + modelId,
      $error = jQuery('#' + errorMessageId);

    // On render - the row isn't exist, so don't cache it.
    jQuery('#' + rowId).removeClass('tooto-error');
    $error.remove();
    tootoPro.ajax.addRequest('theme_builder_conditions_check_conflicts', {
      unique_id: rowId,
      data: {
      ...
      success(data) {
        if (!_.isEmpty(data)) {
          jQuery('#' + rowId).addClass('tooto-error').after('<div id="' + errorMessageId + '" class="tooto-conditions-conflict-message">' + data + '</div>');
        }
      }
    });
  },
...
module.exports = tooto.modules.controls.Repeater.extend({
  ...
  updateActiveRow() {},
  initialize() {
    tooto.modules.controls.Repeater.prototype.initialize.apply(this, arguments);
    this.config = tooto.config.document.theme_builder;
  ...
  onRender() {
    this.ui.btnAddRow.text(__('Add Condition', 'tooto-pro'));
  }
});
...
var inlineControlsStack = __webpack_require__(/*! tooto-pro/editor/inline-controls-stack.js */ "../assets/dev/js/editor/inline-controls-stack.js");
module.exports = inlineControlsStack.extend({
  id: 'tooto-theme-builder-conditions-view',
  template: '#tmpl-tooto-theme-builder-conditions-view',
  childViewContainer: '#tooto-theme-builder-conditions-controls',
  childViewOptions() {
    return {
      elementSettingsModel: this.model
    };
...
class ThemeBuilderSaveAndReload extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-save-and-reload';
  }
  getContainerType() {
    return 'document';
  }
...
class ThemeBuilderUpdatePreviewOptions extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-update-preview-options';
  }
  getContainerType() {
    return 'document';
  }
...
class ThemeBuilderSaveConditions extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-save-conditions';
  }
  getConditions() {
    return !!tooto.config.document.theme_builder;
  }
  apply() {
    const {
      conditionsModel
    } = tootoPro.modules.themeBuilder;
    tootoPro.ajax.addRequest('theme_builder_save_conditions', {
      data: conditionsModel.toJSON({
        remove: ['default']
      }),
      success: () => {
        tooto.config.document.theme_builder.settings.conditions = conditionsModel.get('conditions');
      }
    });
  }
}
...
class ThemeBuilderShowConditions extends $e.modules.hookData.Dependency {
  ...
  getId() {
    return 'tooto-pro-theme-builder-show-conditions';
  }
  getConditions(args) {
    const {
      force = false
    ...
    const themeBuilder = tooto.config.document.theme_builder;
    if (themeBuilder) {
      const hasConditions = themeBuilder.settings.conditions.length,
        hasLocation = themeBuilder.settings.location,
        isDraft = 'draft' === tooto.settings.page.model.get('post_status');
...
class ThemeBuilderPreviewBreak extends $e.modules.hookData.Dependency {
  ...
  getId() {
    return 'tooto-pro-theme-builder-preview-break';
  }
  getConditions(args) {
    ...
    return !!tooto.documents.get(args.id).config.theme_builder;
  }
  apply() {
    return false; // HookBreak.
  }
...
class ThemeBuilderToggleMenuConditions extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-toggle-menu-conditions';
  }
  getContainerType() {
    return 'document';
  }
  ...
  apply() {
    ...
    themeBuilder.ui.menuConditions.toggle(!!tooto.config.document.theme_builder.settings.location);
  }
}
...
class ThemeBuilderRemoveEditorUI extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-remove-editor-ui';
  }
  getConditions(args) {
    const {
      document
  ...
  removePanelFooterSubmenuItems() {
    const footerView = tooto.getPanelView().footer.currentView,
      behavior = footerView._behaviors[Object.keys(footerView.behaviors()).indexOf('saver')];
    tooto.getPanelView().footer.currentView.removeSubMenuItem('saver-options', {
      name: 'conditions'
    });
    behavior.ui.buttonPreview.tipsy('enable').removeClass('tooto-panel-footer-theme-builder-buttons-wrapper tooto-toggle-state');
  }
  removePublishTabs() {
    const component = $e.components.get('theme-builder-publish');
...
class ThemeBuilderAddEditorUI extends $e.modules.hookUI.After {
  ...
  getId() {
    return 'tooto-pro-theme-builder-add-editor-ui';
  }
  getConditions(args) {
    return tooto.documents.get(args.id).config.theme_builder;
  }
  apply() {
    if (tooto.panel) {
      ...
      // First open, the panel is not available yet.
      tooto.once('preview:loaded', this.addUI.bind(this));
    }
  }
  addUI() {
  ...
  addRepeaterControlView() {
    tooto.addControlView('Conditions_repeater', __webpack_require__(/*! ../../../../../conditions/repeater */ "../modules/theme-builder/assets/js/editor/conditions/repeater.js"));
  }
  addPublishTabs() {
    const component = $e.components.get('theme-builder-publish'),
      themeBuilderModuleConfig = tooto.config.document.theme_builder,
      settings = themeBuilderModuleConfig.settings;
    component.manager.conditionsModel = new tootoModules.editor.elements.models.BaseSettings(settings, {
      controls: themeBuilderModuleConfig.template_conditions.controls
    });
    component.addTab('conditions', {
      title: __('Conditions', 'tooto-pro'),
      View: _view.default,
      viewOptions: {
      ...
      name: 'conditions',
      description: __('Apply current template to these pages.', 'tooto-pro'),
      image: tootoPro.config.urls.modules + 'theme-builder/assets/images/conditions-tab.svg'
    });
  }
  addPanelFooterSubmenuItems() {
    const footerView = tooto.getPanelView().footer.currentView,
    ...
    footerView.ui.menuConditions = footerView.addSubMenuItem('saver-options', {
      ...
      name: 'conditions',
      icon: 'eicon-flow',
      title: __('Display Conditions', 'tooto-pro'),
      callback: () => $e.route('theme-builder-publish/conditions')
    });
    footerView.ui.menuConditions.toggle(!!tooto.config.document.theme_builder.settings.location);
    behavior.ui.buttonPreview.tipsy('disable').html(jQuery('#tmpl-tooto-theme-builder-button-preview').html()).addClass('tooto-panel-footer-theme-builder-buttons-wrapper tooto-toggle-state');
  }
}
...
class ThemeBuilderFooterSaverAfterSave extends $e.modules.hookUI.After {
  ...
  getConditions() {
    return tooto.config.document.support_site_editor;
  }
  apply(args, result) {
    const {
      status
  ...
  onPageStatusChange(newStatus) {
    ...
    const options = {
      ...
      message: tooto.config.document.panel.messages.publish_notification,
      buttons: [{
        name: 'open_site_editor',
        text: '<i class="eicon-external-link-square"></i><span class="e-theme-builder-toaster-button-text">' + __('Open Site Editor', 'tooto-pro') + '</span>',
        callback() {
          $e.run('app/open');
        }
      }, {
        name: 'view_live_site',
        text: '<i class="eicon-preview-medium"></i><span class="e-theme-builder-toaster-button-text">' + __('View Live Site', 'tooto-pro') + '</span>',
        callback() {
          open(tooto.config.document.urls.permalink);
        }
      }]
    };
    tooto.notifications.showToast(options);
  }
}
...
class ThemeBuilderModule extends tootoModules.editor.utils.Module {
  __construct() {
    super.__construct(...arguments);
    Object.defineProperty(tootoPro.config, 'theme_builder', {
      get() {
        tootoCommon.helpers.softDeprecated('theme_builder', '2.9.0', 'tooto.config.document.theme_builder');
        return tooto.config.document.theme_builder;
      }
    });
  }
  ontootoLoaded() {
    ...
    tooto.channels.editor.on('tootoProSiteLogo:change', this.openSiteIdentity);
  }
  onDocumentLoaded(document) {
    if (!document.config.theme_builder) {
      return;
    }
    tooto.getPanelView().on('set:page:page_settings', this.updatePreviewIdOptions);
    tooto.channels.editor.on('tootoThemeBuilder:ApplyPreview', this.onApplyPreview);
    tooto.channels.editor.on('page_settings:preview_settings:activated', this.onSectionPreviewSettingsActive);
  }
  onDocumentUnloaded(document) {
    if (!document.config.theme_builder) {
      return;
    }
    tooto.getPanelView().off('set:page:page_settings', this.updatePreviewIdOptions);
    tooto.channels.editor.off('tootoThemeBuilder:ApplyPreview', this.onApplyPreview);
    tooto.channels.editor.off('page_settings:preview_settings:activated', this.onSectionPreviewSettingsActive);
  }
  saveAndReload() {
    $e.run('document/save/auto', {
      force: true,
      onSuccess: () => {
        tooto.dynamicTags.cleanCache();
        const isInitialDocument = tooto.config.initial_document.id === tooto.documents.getCurrentId();
  ...
  updatePreviewIdOptions(render) {
    let previewType = tooto.settings.page.model.get('preview_type');
    if (!previewType) {
      return;
    }
    previewType = previewType.split('/');
    const currentView = tooto.getPanelView().getCurrentPageView(),
...
class Component extends $e.modules.ComponentModalBase {
  ...
  defaultCommands() {
    return {
      ...
      'preview-settings': () => {
        ...
        const panel = tooto.getPanelView();
        $e.route('panel/page-settings/settings');
  ...
  renderTab(tab) {
    ...
    this.layout.modal.getElements('publish').toggleClass('tooto-button-success', isLastTab);
  }
  activateTab(tab) {
...
class _default extends Marionette.LayoutView {
  id() {
    return 'tooto-publish';
  }
  getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-tooto-component-publish');
  }
  regions() {
    return {
      screen: '#tooto-publish__screen'
    };
  }
  templateHelpers() {
    return {
...
exports["default"] = void 0;
class _default extends tootoModules.common.views.modal.Layout {
  getModalOptions() {
    return {
      id: 'tooto-publish__modal',
      hide: {
        onButtonClick: false
      }
    };
  ...
  getLogoOptions() {
    return {
      title: __('Publish Settings', 'tooto-pro')
    };
  }
  initModal() {
    super.initModal();
    ...
    this.modal.addButton({
      name: 'next',
      text: __('Next', 'tooto-pro'),
      callback: () => $e.run('theme-builder-publish/next')
    });
    const $publishButton = this.modal.getElements('publish');
    this.modal.getElements('next').addClass('tooto-button-success').add($publishButton).addClass('tooto-button').removeClass('dialog-button');
  }
}
exports["default"] = _default;
...
module.exports = function () {
  ...
  self.init = function () {
    tooto.hooks.addAction('panel/open_editor/widget/post-comments', self.onPanelShow);
  };
  self.init();
};
...
class Module extends tootoModules.editor.utils.Module {
  ...
  ontootoLoaded() {
    tooto.channels.editor.on('tootoPlaylistWidget:setVideoData', e => {
      $e.run('document/elements/settings', {
...
class WoocommerceCreateWidgetActivateSettingsModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-woocommerce-create-widget-activate-settings-modal';
  }
  getContainerType() {
    return 'column';
  }
  getConditions(args, container) {
    return Object.prototype.hasOwnProperty.call(tootoPro.modules.woocommerce.pageSettingsWidgets, container.model.get('widgetType'));
  }
  apply(args, container) {
    tootoPro.modules.woocommerce.onCreateWidget(container);
  }
}
...
class WoocommerceDeleteWidgetDeactivateSettingsModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-woocommerce-delete-widget-deactivate-settings-modal';
  }
  getContainerType() {
    return 'widget';
  }
  ...
  apply(args, container) {
    tootoPro.modules.woocommerce.onDeleteWidget(container);
  }
}
...
class WoocommerceNotices extends $e.modules.hookData.After {
  ...
  getConditions(args) {
    return 'kit' === tooto.documents.getCurrent().config.type && Array.isArray(args.settings.woocommerce_notices_elements);
  }
  apply(args) {
    const {
      woocommerce
...
class WoocommerceSaveShowModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'tooto-pro-woocommerce-save-show-modal';
  }
  getConditions(args) {
    return args.status && -1 !== ['private', 'publish'].indexOf(args.status);
  }
  apply() {
    tootoPro.modules.woocommerce.onUpdateDocument();
  }
}
...
class WoocommerceModule extends tootoModules.editor.utils.Module {
  constructor() {
    super(...arguments);
    this.pageSettingsWidgets = {
      'woocommerce-checkout-page': {
        headerMessage: __('Want to save this as your checkout page?', 'tooto-pro'),
        message: __('Changes you make here will override your existing WooCommerce settings.', 'tooto-pro'),
        confirmMessage: __('You\'ve updated your checkout page.', 'tooto-pro'),
        cancelMessage: __('<h3>Set up a checkout page</h3><br>Without a checkout page, visitors can\'t complete transactions on your site. To set one up, go to Site Settings.', 'tooto-pro'),
        failedMessage: __('<h3>Sorry, something went wrong.</h3><br>To define a checkout page for your site, head over to Site Settings.', 'tooto-pro'),
        optionName: 'woocommerce_checkout_page_id',
        woocommercePageName: 'checkout'
      },
      'woocommerce-cart': {
        headerMessage: __('Want to save this as your cart page?', 'tooto-pro'),
        message: __('Changes you make here will override your existing WooCommerce settings.', 'tooto-pro'),
        confirmMessage: __('You\'ve updated your cart page.', 'tooto-pro'),
        cancelMessage: __('<h3>Set up a cart page</h3><br>The cart page shows an order summary. To set one up, go to Site Settings.', 'tooto-pro'),
        failedMessage: __('<h3>Sorry, something went wrong.</h3><br>To define a cart page for your site, head over to Site Settings.', 'tooto-pro'),
        optionName: 'woocommerce_cart_page_id',
        woocommercePageName: 'cart'
      },
      'woocommerce-my-account': {
        headerMessage: __('Want to save this as your my account page?', 'tooto-pro'),
        message: __('Changes you make here will override your existing WooCommerce settings.', 'tooto-pro'),
        confirmMessage: __('You\'ve updated your my account page.', 'tooto-pro'),
        cancelMessage: __('<h3>Set up a My Account page</h3><br>Without it, customers can\'t update their billing details, review past orders, etc. To set up My Account, go to Site Settings.', 'tooto-pro'),
        failedMessage: __('<h3>Sorry, something went wrong.</h3><br>To define a my account page for your site, head over to Site Settings.', 'tooto-pro'),
        optionName: 'woocommerce_myaccount_page_id',
        woocommercePageName: 'myaccount'
      },
      'woocommerce-purchase-summary': {
        headerMessage: __('Want to save this as your purchase summary page?', 'tooto-pro'),
        message: __('Changes you make here will override your WooCommerce default purchase summary page.', 'tooto-pro'),
        confirmMessage: __('You\'ve updated your summary page.', 'tooto-pro'),
        cancelMessage: __('<h3>Set up a purchase summary page</h3><br>This page shows payment and order details. To set one up, go to Site Settings.', 'tooto-pro'),
        failedMessage: __('<h3>Sorry, something went wrong.</h3><br>To define a purchase summary page for your site, head over to Site Settings.', 'tooto-pro'),
        optionName: 'tooto_woocommerce_purchase_summary_page_id',
  ...
  ontootoInit() {
    tooto.hooks.addAction('editor/widgets/loop-grid/on-init', this.addWooCommerceClassToLoopWrapper);
  }
  ontootoFrontendInit() {
    tootoFrontend.elements.$body.on('added_to_cart', (e, data) => {
    ...
    if ('loop-item' === tooto.documents.currentDocument.config.type && 'product' === tooto.documents.currentDocument.config.settings.settings.source) {
      // Add the 'woocommerce' class to the Loop document wrapper only when editing a Product Loop Template in the
      // theme builder.
      tooto.on('document:loaded', () => {
        tooto.$previewContents[0].querySelector('.e-loop-item').classList.add('woocommerce');
      });
    }
  }
  ...
  ontootoLoaded() {
    ...
    for (const section of noticeSections) {
      tooto.channels.editor.on('kit_settings:' + section + ':activated', () => {
        this.renderMockNotices(tooto.documents.getCurrent().container.settings.get('woocommerce_notices_elements'));
      });
    }

    // Custom Empty Cart Template.
    tooto.channels.editor.on('editor:widget:woocommerce-cart:section_additional_options:activated', () => {
      this.onTemplateIdChange('additional_template_select');
    });

    // Custom My Account Dashboard Template
    tooto.channels.editor.on('editor:widget:woocommerce-my-account:section_additional_options:activated', () => {
      this.onTemplateIdChange('customize_dashboard_select');
    });
  }
  renderMockNotices(noticeElements) {
    const noticesWrapper = tooto.$previewContents.find('.woocommerce-notices-wrapper');
    ...
    tootoFrontend.elements.$body.addClass(noticesClass.trim());
    noticesWrapper.addClass('tooto-loading');
    // Wait for the Ajax call to finish before the select2 can be changed again.
    jQuery('.tooto-select2').attr('disabled', 'disabled');
    tootoPro.ajax.addRequest('woocommerce_mock_notices', {
      data: {
        notice_elements: noticeElements
      },
      success(data) {
        noticesWrapper.remove();
        tooto.$previewContents.find('.tooto-editor-preview').prepend(data);
        noticesWrapper.removeClass('tooto-loading');
        // Enable the select2 again.
        jQuery('.tooto-select2').removeAttr('disabled');
      }
    });
  }
  onTemplateIdChange(sectionActive) {
    const editor = tooto.getPanelView().getCurrentPageView(),
      ...
      $editButton = editor.$el.find('.tooto-edit-template');
    if (!templateID) {
      $editButton.addClass('e-control-tool-disabled').hide();
    } else {
      const editUrl = tootoConfig.home_url + '?p=' + templateID + '&tooto';
  ...
  onUpdateDocument() {
    ...
    tootoFrontend.elements.$body.trigger('added_to_cart', [{
    ...
    const saveWoocommercePageSettingKeys = Object.keys(this.createdPageSettingsWidgets),
      ...
      postId = tooto.documents.getCurrent().id;
    ...
    if (postId === tootoPro.config.woocommerce.woocommercePages[lastWidgetCreatedOptions.woocommercePageName]) {
      return;
    }
    tootoCommon.dialogsManager.createWidget('confirm', {
      id: 'tooto-woocommerce-save-pages',
      ...
      strings: {
        confirm: __('Save', 'tooto-pro'),
        cancel: __('No thanks', 'tooto-pro')
      },
  ...
  onConfirmModal(lastWidgetCreatedOptions) {
    tootoPro.ajax.addRequest('woocommerce_update_page_option', {
      data: {
        option_name: lastWidgetCreatedOptions.optionName
      },
      success: () => {
        tooto.notifications.showToast({
  ...
  showPagesSettingsToast(message) {
    const buttons = [];
    tooto.notifications.initToast();
    buttons.push({
      name: 'take_me_there',
      text: __('Take me there', 'tooto'),
  ...
  openSiteSettingsTab() {
    ...
    const isWPPreviewMode = tootoCommon.elements.$body.hasClass('tooto-editor-preview');
    if (isWPPreviewMode) {
      tooto.exitPreviewMode();
    }
    const isInSettingsPanelActive = 'panel/global/menu' === tooto.documents.currentDocument.config.panel.default_route;
    if (isInSettingsPanelActive) {
      $e.run('panel/global/close');
      return;
    }
    $e.run('editor/documents/switch', {
      id: tooto.config.kit_id,
      mode: 'autosave'
    }).then(() => {
      if (tabId) {
    ...
    .then(() => {
      if (sectionId) {
        const sectionElement = jQuery('.tooto-control-' + sectionId);
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "tooto-pro:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunktooto_pro"] = self["webpackChunktooto_pro"] || [];
...
var tootoPro = Marionette.Application.extend({
  config: {},
  modules: {},
  initModules() {
    ...
    if (tootoCommon.config.experimentalFeatures['page-transitions']) {
    ...
    if (tootoCommon.config.experimentalFeatures['mega-menu']) {
      tootoCommon.elements.$window.on('tooto/nested-element-type-loaded', async () => {
  ...
  ajax: {
    ...
    send() {
      return tootoCommon.ajax.send.apply(tootoCommon.ajax, this.prepareArgs(arguments));
    },
    addRequest() {
      return tootoCommon.ajax.addRequest.apply(tootoCommon.ajax, this.prepareArgs(arguments));
    }
  },
  ...
  onStart() {
    this.config = tootoProEditorConfig;
    this.initModules();
    jQuery(window).on('tooto:init', () => this.ontootoInit()).on('tooto/connect/success/editor-pro-activate', this.onActivateSuccess);
  },
  ontootoInit() {
    tooto.on('preview:loaded', () => this.ontootoPreviewLoaded());
    tootoPro.libraryRemoveGetProButtons();
    tootoCommon.debug.addURLToWatch('tooto-pro/assets');
  },
  ontootoPreviewLoaded() {
    tooto.$preview[0].contentWindow.tootoPro = this;
  },
  libraryRemoveGetProButtons() {
    tooto.hooks.addFilter('tooto/editor/template-library/template/action-button', function (viewID, templateData) {
      var _tooto$config, _tooto$config$lib;
      if (templateData.accessLevel === undefined || ((_tooto$config = tooto.config) === null || _tooto$config === void 0 ? void 0 : (_tooto$config$lib = _tooto$config.library_connect) === null || _tooto$config$lib === void 0 ? void 0 : _tooto$config$lib.current_access_level) === undefined) {
        // BC support.
        return templateData.isPro && !tootoPro.config.isActive ? '#tmpl-tooto-pro-template-library-activate-license-button' : '#tmpl-tooto-template-library-insert-button';
      }

      // When the template should be at least "pro" and the license is not active.
      if (templateData.accessLevel > 0 && !tootoPro.config.isActive) {
        return '#tmpl-tooto-pro-template-library-activate-license-button';
      }
      ...
      if (templateData.accessLevel > tooto.config.library_connect.current_access_level) {
        return viewID;
      }

      // When the current license can insert the template.
      return '#tmpl-tooto-template-library-insert-button';
    });
  },
  onActivateSuccess() {
    // Hide notice.
    tooto.noticeBar.onCloseClick();

    // Mark site connect for insert templates connect screen.
    tooto.config.library_connect.is_connected = true;

    // Mark pro is active - for `this.libraryRemoveGetProButtons`.
    tootoPro.config.isActive = true;
    tooto.notifications.showToast({
      message: __('Connected Successfully', 'tooto')
    });
  }
});
window.tootoPro = new tootoPro();
tootoPro.start();
})();
