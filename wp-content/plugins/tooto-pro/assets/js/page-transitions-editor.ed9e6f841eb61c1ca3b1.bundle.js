...
"use strict";
(self["webpackChunktooto_pro"] = self["webpackChunktooto_pro"] || []).push([["page-transitions-editor"],{
...
class Animate extends $e.modules.CommandBase {
   ...
   */
  apply() {
    const pageTransition = tooto.$previewContents[0].querySelector('e-page-transition');
    if (!pageTransition) {
      return;
    }
...
class PageTransitionPreview {
  ...
  run(component, route) {
    if ('panel/global/settings-page-transitions' === route) {
      (0, _utils.renderPageTransition)(tooto.documents.getCurrent().container);
  ...
  togglePageTransitionPreview() {
    ...
    const className = 'e-page-transition--preview',
      pageTransition = tooto.$previewContents[0].body.querySelector('e-page-transition');
    if (!pageTransition) {
      return;
    }
...
function renderPageTransition(container) {
  let pageTransition = tooto.$previewContents[0].querySelector('e-page-transition');
  ...
  if (!pageTransition) {
    ...
    tooto.$previewContents[0].body.append(pageTransition);
  }
...
class _default extends tootoModules.editor.utils.Module {
   ...
   */
  bindEvents() {
    // Make sure that `window.tooto` is initialized.
    // TODO: Find a better solution. It's caused because of the dynamic import.
    if (window.tooto) {
      this.onAnimateButtonClick();
      return;
    }
    jQuery(window).on('tooto:init', () => this.onAnimateButtonClick());
  }
  ...
  onAnimateButtonClick() {
    tooto.channels.editor.on('tootoPageTransitions:animate', () => {
      $e.run('page-transitions/animate');
    });
  }
}
