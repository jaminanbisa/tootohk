...
(self["webpackChunktooto_pro"] = self["webpackChunktooto_pro"] || []).push([["video-playlist"],{
...
// Copied from the core, original path: tooto/assets/dev/js/frontend/handlers/base-tabs.js.
class baseTabs extends tootoModules.frontend.handlers.Base {
  ...
  activateTab(tabIndex) {
    ...
    $requestedContent[settings.showTabFn](animationDuration, () => tootoFrontend.elements.$window.trigger('resize'));
...
async function triggerEvent(widgetObject, eventType, eventTrigger, positionInVideoList) {
  ...
  jQuery('body').trigger('tooto-video-playList', currentEvent);
}
...
class VideoPlaylistHandler extends _baseTabs.default {
  ...
  onInit(...args) {
    ...
    const storageObject = tootoFrontend.storage.get(this.storageKey);
  ...
  handleFirstVideoActivation(isLazyLoad) {
    ...
    const playlistElement = document.querySelector('.tooto-element-' + this.playlistId + ' .e-tabs-main-area'),
          observer = tootoModules.utils.Scroll.scrollObserver({
      callback: event => {
      ...
      playListItem.playerInstance.create().then(() => {
        ...
        playListItem.playerInstance.handleEnded(() => {
          ...
          if (!this.watchedVideosArray.includes(endedVideoId) && this.watchedIndication) {
            ...
            tootoFrontend.storage.set(this.storageKey, JSON.stringify(this.watchedVideosArray));
