...
/* global tootoScreenshotConfig */
class Screenshot extends tootoModules.ViewModule {
  getDefaultSettings() {
    return {
      ...
      timerLabel: null,
      timer_label: `${tootoScreenshotConfig.post_id} - timer`,
      image_placeholder: 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs=',
      isDebug: tootoCommonConfig.istootoDebug,
      isDebugSvg: false,
      ...tootoScreenshotConfig
    };
  }
  getDefaultElements() {
    const $tooto = jQuery(tootoScreenshotConfig.selector);
    const $sections = $tooto.find('.tooto-section-wrap > .tooto-section, .tooto > .tooto-section');
    return {
      $tooto,
      $sections,
      $firstSection: $sections.first(),
      $nottootoElements: tootoCommon.elements.$body.find('> *:not(style, link)').not($tooto),
      $head: jQuery('head')
    };
  }
  onInit() {
   ...
   */
  captureScreenshot() {
    if (!this.elements.$tooto.length) {
      tootoCommon.helpers.consoleWarn('Screenshots: The content of this page is empty, the module will create a fake conent just for this screenshot.');
   ...
   */
  createFakeContent() {
    this.elements.$tooto = jQuery('<div>').css({
    ...
    this.elements.$tooto.append(jQuery('<h1>').css({
      fontSize: '85px'
    }).html(this.getSettings('empty_content_headline')));
    document.body.prepend(this.elements.$tooto);
  }
   ...
   */
  handleIFrames() {
    this.elements.$tooto.find('iframe').each((index, el) => {
  ...
  removeUnnecessaryElements() {
    ...
    this.elements.$nottootoElements.remove();
  }
   ...
   */
  handleLinks() {
    tootoCommon.elements.$body.find('a').attr('href', '/');
  }
  ...
  download(uri) {
    ...
    tootoCommon.elements.$body.append($link);
    $link.trigger('click');
  }
  ...
  save(canvas) {
    return new Promise((resolve, reject) => {
      tootoCommon.ajax.addRequest('screenshot_save', {
        data: {
  ...
  markAsFailed() {
    return new Promise((resolve, reject) => {
      tootoCommon.ajax.addRequest('screenshot_failed', {
        data: {
          post_id: this.getSettings('post_id')
        },
        success: () => {
