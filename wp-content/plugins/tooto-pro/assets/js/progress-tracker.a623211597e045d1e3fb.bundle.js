...
"use strict";
(self["webpackChunktooto_pro"] = self["webpackChunktooto_pro"] || []).push([["progress-tracker"],{
...
class CircularProgress {
  constructor(element, settings) {
    ...
    this.circularProgressTracker = element.find('.tooto-scrolling-tracker-circular')[0];
...
class LinearProgress {
  constructor(element, settings) {
    this.settings = settings;
    this.linearProgressTracker = element.find('.tooto-scrolling-tracker-horizontal')[0];
...
class ProgressTracker extends tootoModules.frontend.handlers.Base {
  onInit() {
  ...
  getTrackingElementSelector() {
    ...
    switch (trackingElementSetting) {
      case 'selector':
        ...
        break;
      case 'post_content':
        selector = jQuery('.tooto-widget-theme-post-content');
        break;
      default:
        selector = this.isScrollSnap() ? jQuery('#e-scroll-snap-container') : tootoFrontend.elements.$body;
        break;
    }
    return selector;
  }

  // TODO: On tooto-Pro-3.6.0 delete this function and instead
  // use the function isScrollSnapActivated() from \tooto\assets\dev\js\frontend\utils\utils.js
  isScrollSnap() {
    const scrollSnapStatus = this.isEdit ? tooto.settings.page.model.attributes.scroll_snap : tootoFrontend.config.settings.page.scroll_snap;
  ...
  scrollHandler() {
    ...
    const $trackingElementSelector = this.getTrackingElementSelector(),
      scrollStartPercentage = $trackingElementSelector.is(tootoFrontend.elements.$body) || $trackingElementSelector.is(jQuery('#e-scroll-snap-container')) ? -100 : 0;
    this.progressPercentage = tootoModules.utils.Scroll.getElementViewportPercentage(this.getTrackingElementSelector(), {
  ...
  initListeners() {
    ...
    tootoFrontend.elements.$body[0].addEventListener('scroll', this.handler);
  }
  onDestroy() {
    ...
    tootoFrontend.elements.$body[0].removeEventListener('scroll', this.handler);
  }
}
