import { Button, CardHeader, Heading, Icon, Text } from '@tooto/app-ui';
...
export const SiteTemplateHeader = ( props ) => {
	const status = props.status && 'publish' !== props.status ? ` (${ props.status })` : '',
		...
		ActionButtons = () => (
			<>
				<Button text={ __( 'Edit', 'tooto-pro' ) } icon="eicon-edit" className="e-site-template__edit-btn" size="sm" url={ props.editURL } />
