...
var sideEffectL10nManipulation = function sideEffectL10nManipulation(status) {
    ...
    if (node) {
      if (statusLabel && ('publish' != status)) {
        ...
        if ($('#ppcs_save_draft_label').filter(':visible').length) {
          ...
          $('#ppcs_save_draft_label').html(rvycs__('Save as %s').replace('%s', statusLabel)).css('left', spanPosLeft + 'px');
        }
      }
    }
  //}, 100);
  ...
  if (-1 == PPCustomStatuses.publishedStatuses.indexOf(status)) {
    $('div.publishpress-extended-post-status-note').html(rvycs__("This will override all status settings above.", 'publishpress')).show();
  } else {
    $('div.publishpress-extended-post-status-note').html(rvycs__('To select a workflow status, switch to Draft first.', 'publishpress')).show();
  }
};
...
var PPCustomPostStatusInfo = function PPCustomPostStatusInfo(_ref) {
  ...
  }, React.createElement("h4", null, rvycs__('Post Status', 'publishpress')), React.createElement(SelectControl, {
    label: "",
    value: status,
