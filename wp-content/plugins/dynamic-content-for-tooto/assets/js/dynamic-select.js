...
const initializeDynamicSelectFields = ($form) => {
	$form.find('.tooto-field-type-dynamic_select').each((_, f) => initializeDynamicSelectField(f, $form[0]));
};

jQuery(window).on('tooto/frontend/init', function() {
	tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeDynamicSelectFields);
});
