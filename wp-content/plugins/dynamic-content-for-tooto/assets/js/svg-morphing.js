( function( $ ) {
	...
	var WidgetElements_SvgMorphHandler = function( $scope, $ ) {
		...
		*/
		...
		var createTween = function(){
			...
			is_running = true;
			if( run == 'paused' && tootoFrontend.isEditMode() ){
				ferma();
			}
		...
		if(!tootoFrontend.isEditMode() && contentElemsTotal > 1 && elementSettings.svg_trigger == 'animation'){
		...
		if(tootoFrontend.isEditMode()){
			if(transitionTl) transitionTl.kill($(dceshape));
			if(transitionTlpos) transitionTlpos.kill($(dceshape_svg));
			$('.tooto-element[data-id='+id_scope+']').off('mouseenter','svg');
			$('.tooto-element[data-id='+id_scope+']').off('mouseleave','svg');
			$('.tooto-element[data-id='+id_scope+']').off('touchstart','svg');
			$('.tooto-element[data-id='+id_scope+']').off('touchend','svg');
		}
		...
		setTimeout(function(){
			...
			if(elementSettings.svg_trigger == 'animation'){
				...
				if( tootoFrontend.isEditMode() && contentElemsTotal > 1) playShapeEl();
				...
				$('.tooto-element[data-id='+id_scope+']').on('mouseenter','svg', mouseenterFn);
				$('.tooto-element[data-id='+id_scope+']').on('mouseleave','svg', mouseleaveFn);
				$('.tooto-element[data-id='+id_scope+']').on('touchstart','svg', mouseenterFn);
				$('.tooto-element[data-id='+id_scope+']').on('touchend','svg', mouseleaveFn);
	...
	$( window ).on( 'tooto/frontend/init', function() {
		tootoFrontend.hooks.addAction( 'frontend/element_ready/dyncontel-svgmorphing.default', WidgetElements_SvgMorphHandler );
	} );
} )( jQuery );
