...
(function ($) {
    ...
    var dropCap = function ($target) {
        if ($target.length) {
            ...
            if ($first_p.charAt(0) != '<') {
                // not a html tag
                $target.html('<span class="tooto-drop-cap"><span class="tooto-drop-cap-letter">' + $first_p.charAt(0) + '</span></span>' + $first_p.substring(1));
            } else {
                $first_p = $target.find('*:first-child').html().trim();
                $target.find('*:first-child').html('<span class="tooto-drop-cap"><span class="tooto-drop-cap-letter">' + $first_p.charAt(0) + '</span></span>' + $first_p.substring(1));
            }
        }
    };
    var WidgetElementsACFDCEHandler = function ($scope, $) {
        ...
        var bindEvents = function () {
            $scope.find('.tooto-custom-embed-image-overlay').on('click', handleVideo);
        };
    ...
    $(window).on('tooto/frontend/init', function () {
        tootoFrontend.hooks.addAction('frontend/element_ready/dyncontel-acf.default', WidgetElementsACFDCEHandler);
    });
})(jQuery);
