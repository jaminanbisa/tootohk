...
(function ($) {
    ...
    var smoothTransitionHandler = function ( ) {
        ...
        var a_class = settings_global.a_class || 'a:not([target="_blank"]):not([href=""]):not([href^="uploads"]):not([href^="#"]):not([href^="mailto"]):not([href^="tel"]):not(.no-transition):not(.gallery-lightbox):not(.tooto-clickable):not(.oceanwp-lightbox):not(.is-lightbox):not(.tooto-icon):not(.download-link):not([href^="tooto-action"])';
        if (typeof tootoFrontend !== "undefined" && tootoFrontend.isEditMode()) {
    ...
    function handleLoadingMode(newValue) {
        ...
        if (newValue) {
            ...
            loading_style = 'loading-style-' + tooto.settings.dynamicooo.model.attributes.smoothtransition_loading_style;
    ...
    function handleAnimsition(newValue) {
        ...
        if (newValue) {
            ...
            if (! is_animsition) {
                settings_global = tooto.settings.dynamicooo.model.attributes;
            }
    ...
    $(window).on('tooto/frontend/init', function () {
        ...
        if (tootoFrontend.isEditMode()) {
            ...
            if (tooto.settings.dynamicooo) {
                tooto.settings.dynamicooo.addChangeCallback('enable_smoothtransition', handleAnimsition);
                tooto.settings.dynamicooo.addChangeCallback('dce_smoothtransition_class_controller', handleAnimsition);
                tooto.settings.dynamicooo.addChangeCallback('smoothtransition_enable_overlay', handleOverlay);
                tooto.settings.dynamicooo.addChangeCallback('smoothtransition_loading_mode', handleLoadingMode);
                tooto.settings.dynamicooo.addChangeCallback('smoothtransition_loading_style', handleLoadingStyle);
                tooto.settings.dynamicooo.addChangeCallback('enable_trackerheader', handleHeaderTracker);
                tooto.settings.dynamicooo.addChangeCallback('dce_trackerheader_class_controller', handleHeaderTracker);
                tooto.settings.dynamicooo.addChangeCallback('trackerheader_overlay', handleTrackerHeader_Overlay);
            }
        }
    ...
    $(function () {
        ...
        if (typeof tootoFrontendConfig !== "undefined") {
            settings_global = tootoFrontendConfig.settings.dynamicooo;
        } else {
        ...
        if (settings_global) {
            ...
            var deviceMode = $('body').attr('data-tooto-device-mode') || 'desktop';
