	...
	function ( $ ) {
		var WidgetElements_SvgDistortionHandler = function ( $scope,$ ) {
			...
			if ( tootoFrontend.isEditMode() ) {
				...
				$( '.tooto-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
				   ' a' ).off( 'mouseenter' );
				$( '.tooto-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
				   ' a' ).off( 'mouseleave' );
				$( '.tooto-element[data-id=' + id_scope + '] svg, .' + elementSettings.id_svg_class +
			...
			if ( elementSettings.svg_trigger == 'animation' ) {
				...
				if ( tootoFrontend.isEditMode() ) playShapeEl();
		...
		$( window ).on( 'tooto/frontend/init',function () {
			tootoFrontend.hooks.addAction( 'frontend/element_ready/dyncontel-svgdistortion.default',WidgetElements_SvgDistortionHandler );
		} );
	}
)( jQuery );
