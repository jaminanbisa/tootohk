...
function initializeStripeField(wrapper, $scope) {
	...
	const $submitButton = $scope.find('.tooto-field-type-submit button');
...
function initializeAllStripeFields($scope) {
	$scope.find('.tooto-field-type-dce_form_stripe').each((_, w) => initializeStripeField(w, $scope));
}

jQuery(window).on('tooto/frontend/init', function() {
	tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllStripeFields);
});
