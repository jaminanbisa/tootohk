...
function initializeAllAmountFields($scope) {
	$scope.find('.tooto-field-type-amount').each((_, w) => initializeAmountField(w, $scope[0]));
}
...
jQuery(window).on('tooto/frontend/init', function() {
	...
	tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllAmountFields);
});
