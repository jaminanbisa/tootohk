...
(function ($) {
    var WidgetDyncontel_ACFRepeaterHandler = function ($scope, $) {
        ...
        if (elementSettings.dce_acf_repeater_format == 'accordion') {
            $scope.find('.tooto-tab-title').on('click', function(){
                if (elementSettings.dce_acf_repeater_accordion_close) {
                    $scope.find('.tooto-active').not(this).each(function() {
                        jQuery(this).toggleClass('tooto-active');
                        jQuery(this).next().slideToggle().toggleClass('tooto-active');
                    });
                }
                jQuery(this).toggleClass('tooto-active');
                jQuery(this).next().slideToggle().toggleClass('tooto-active');
        ...
        if (elementSettings.dce_acf_repeater_format == 'masonry') {
            ...
            var swiperOptions = {
                ...
                loop: cicloInfinito, 
                navigation: {
                    nextEl: id_post ? '.dce-tooto-post-'+id_post+' .tooto-element-' + id_scope + ' .next-' + counter_id : '.next-' + counter_id,
                    prevEl: id_post ? '.dce-tooto-post-'+id_post+' .tooto-element-' + id_scope + ' .prev-' + counter_id : '.prev-' + counter_id,
                },
                pagination: {
                    el: id_post ? '.dce-tooto-post-'+id_post+' .tooto-element-' + id_scope + ' .pagination-' + counter_id : '.pagination-' + counter_id, //'.swiper-pagination', //'.pagination-acfslider-'+id_scope,
                    clickable: true,
...

			{
				...
				if (!isCarousel) {
					...
					let tootoTab = elementSwiper.closest('.tooto-tab-content');
					if (tootoTab && elementSettings.useAutoplay) { // we are inside a tab, autoplay needs to be updated.
						manageAutoplayInsideTab( tootoTab, swiperOptions, elementSwiper );
					} else {
						initializeSwiper(swiperOptions, elementSwiper);
					}
				}
    ...
    $(window).on('tooto/frontend/init', function () {
        tootoFrontend.hooks.addAction('frontend/element_ready/dce-acf-repeater-v2.default', WidgetDyncontel_ACFRepeaterHandler);
    });
