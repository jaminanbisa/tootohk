...
if(!is_animate)
setTimeout(function(){if(!tootoFrontend.isEditMode()){is_animate=!0;$('body').removeClass('modal-close-'+id_modal).removeClass('modal-close-'+id_modal_scope);$('body').addClass('modal-open-'+id_modal).addClass('modal-open-'+id_modal_scope).addClass('dce-modal-open');$('html').addClass('dce-modal-open');$('#'+id_modal+' .modal-dialog').one('webkitAnimationEnd oanimationend msAnimationEnd animationend',function(el){is_animate=!1})}
...
$('.tooto-video').each(function(){this.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}','*')});$('.tooto-video-iframe').each(function(){this.contentWindow.postMessage('{"method":"pause"}','*')});$('body').removeClass('modal-open-'+id_modal).removeClass('modal-open-'+id_modal_scope);$('body').addClass('modal-close-'+id_modal).addClass('modal-close-'+id_modal_scope);$('#'+id_modal+'-background').hide();$('#'+id_modal+' .modal-dialog').one('webkitAnimationEnd oanimationend msAnimationEnd animationend',function(el){$('#'+id_modal).hide();setTimeout(function(){if(!tootoFrontend.isEditMode()){$('body').removeClass('modal-close-'+id_modal).removeClass('modal-close-'+id_modal_scope).removeClass('dce-modal-open');$('html').removeClass('dce-modal-open');$(el.currentTarget).off('webkitAnimationEnd oanimationend msAnimationEnd animationend')}
...
var modal=$scope.find('.dce-popup-container-'+id_scope);function push_actions(){if(!tootoFrontend.isEditMode()){$(modal).prependTo("body")}}
...
$(window).on('scroll',function(){$('.modal-hide-on-scroll:visible').each(function(){$(this).removeClass('modal-hide-on-scroll');dce_hide_modal($(this).attr('id'))})});$(document).on('keyup',function(evt){if(evt.keyCode==27){$('.modal-hide-esc:visible').each(function(){dce_hide_modal($(this).attr('id'))})}});$(document).on('click','#'+id_popup+'.dce-modal .dce-modal-close, .dce-modal .dce-button-close-modal, .dce-modal .dce-button-next-modal',function(){dce_hide_modal(id_popup)});$(document).on('click','#'+id_popup+'-background.dce-modal-background-layer-close',function(){dce_hide_modal(id_popup)})};$(window).on('tooto/frontend/init',function(){tootoFrontend.hooks.addAction('frontend/element_ready/dyncontel-popup.default',WidgetElementsPopupHandler)})})(jQuery)