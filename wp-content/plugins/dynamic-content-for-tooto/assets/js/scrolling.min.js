...
setWidth(){heightAdminBar=0;if($('body').is('.admin-bar')&&!tootoFrontend.isEditMode()){heightAdminBar=45}
...
setHeight(){heightAdminBar=0;if($('body').is('.admin-bar')&&!tootoFrontend.isEditMode()){heightAdminBar=45}
...
var is_inertiaScroll=!1;var directionScroll='vertical';var coefSpeed_inertiaScroll=0.05;var html=document.documentElement;var scroller={};const body=document.body;var main={};var mainWrap={};var skew_inertiaScroll=20;var bounce_inertiaScroll=0;let sx=0;let sy=0;let dx=sx;let dy=sy;var requestId;var init_Scrollify=function(){$('body').addClass('dce-scrollify dce-scrolling');if(settings_page.custom_class_section_sfy){$customClass=settings_page.custom_class_section_sfy;if('.'===settings_page.custom_class_section_sfy[0]){$customClass=settings_page.custom_class_section_sfy.substring(1)}}else{$customClass='tooto-section:not(.tooto-inner-section)'}
$target_sections='.tooto-'+currentPostId;if(!$target_sections)
$target_sections='';sezioni=$target_sections+'.tooto .tooto-section-wrap .'+$customClass;wrapperSezioni=$($target_sections+'.tooto .tooto-section-wrap');$($target_sections).addClass('scroll-direction-'+settings_page.directionScroll);$.scrollify({section:sezioni,sectionName:'id',interstitialSection:settings_page.interstitialSection,easing:"easeOutExpo",scrollSpeed:Number(settings_page.scrollSpeed.size)||1100,offset:Number(settings_page.offset.size)||0,scrollbars:Boolean(settings_page.scrollBars),setHeights:Boolean(settings_page.setHeights),overflowScroll:Boolean(settings_page.overflowScroll),updateHash:Boolean(settings_page.updateHash),touchScroll:Boolean(settings_page.touchScroll),before:function(i,panels){var ref=panels[i].attr("data-id");$(".dce-scrollify-pagination .nav__item--current").removeClass("nav__item--current");$(".dce-scrollify-pagination").find("a[href=\"#"+ref+"\"]").addClass("nav__item--current")},afterRender:function(){is_scrollify=!0;if(settings_page.enable_scrollify_nav||tootoFrontend.isEditMode()){var scrollify_pagination='';createNavigation(settings_page.snapscroll_nav_style);$("body").on("click",".dce-scrollify-pagination a",function(){$.scrollify.move($(this).attr("href"));return!1});if(!Boolean(settings_page.enable_scrollify_nav)){handleScrollify_enablenavigation('')}
...
newPagination+='<li><a href="#'+$(this).attr("data-id")+'" class="'+activeClass+' nav__item" aria-label="'+(i+1)+'">'+$itemInner+$itemTitle+'</a></li>'}});newPagination+="</ul>";$("body").append(newPagination)};var init_ScrollEffects=function(){$('body').addClass('dce-pageScroll dce-scrolling');if(settings_page.custom_class_section){$customClass=settings_page.custom_class_section}else{$customClass='tooto-section'}
$target_sections='.tooto-'+currentPostId;if(!$target_sections)
$target_sections='';sezioni=$target_sections+'.tooto .tooto-section-wrap > .'+$customClass;sectionsAvailable=$(sezioni);wrapperSezioni=$($target_sections+'.tooto .tooto-section-wrap');$($target_sections).addClass('scroll-direction-'+settings_page.directionScroll);animationType=settings_page.animation_effects||['spin'];var animationType_string=[];if(animationType.length)
...
lax.update(window.scrollY);requestId=window.requestAnimationFrame(updateLax)};requestId=window.requestAnimationFrame(updateLax);is_scrollEffects=!0};var init_InertiaScroll=function(){if(settings_page.custom_class_section){$customClass=settings_page.custom_class_section}else{$customClass='tooto-section'}
...
directionScroll=settings_page.directionScroll||'vertical';$target_sections='.tooto-'+currentPostId;if(!$target_sections)
$target_sections='';sezioni=$target_sections+'.tooto .tooto-section-wrap .'+$customClass;sectionsAvailable=$(sezioni);wrapperSezioni=$($target_sections+'.tooto .tooto-section-wrap');if($('.tooto-template-canvas').length){main=document.querySelector($target_sections);mainWrap=document.querySelector($target_sections+'.tooto .tooto-section-wrap')}else{if(settings_page.automatic_wrapper){if(!$('#outer-wrap').length)
...
setTimeout(function(){if(settings_page.enable_inertiaScroll)
init_InertiaScroll()},100)}else{removeInertiaScroll()}}
$(window).on('tooto/frontend/init',function(){if(typeof tootoFrontendConfig.settings.page!=='undefined'){settings_page=tootoFrontendConfig.settings.page;currentPostId=tootoFrontendConfig.post.id;is_enable_dceScrolling=settings_page.enable_dceScrolling;is_enable_scrollify=settings_page.enable_scrollify;is_enable_scrollEffects=settings_page.enable_scrollEffects;is_enable_inertiaScroll=settings_page.enable_inertiaScroll;var responsive_scrollEffects=settings_page.responsive_scrollEffects;var responsive_snapScroll=settings_page.responsive_snapScroll;var responsive_inertiaScroll=settings_page.responsive_inertiaScroll;var deviceMode=$('body').attr('data-tooto-device-mode');if(is_enable_scrollEffects&&is_enable_dceScrolling&&$.inArray(deviceMode,responsive_scrollEffects)>=0){init_ScrollEffects()}
...
if(tootoFrontend.isEditMode()){if(tooto){settings_page=tooto.settings.page.model.attributes;tooto.settings.page.addChangeCallback('enable_dceScrolling',handlescroll_viewport);tooto.settings.page.addChangeCallback('enable_scrollify',handleScrollify);tooto.settings.page.addChangeCallback('scrollSpeed',handleScrollify_speed);tooto.settings.page.addChangeCallback('offset',handleScrollify_offset);tooto.settings.page.addChangeCallback('ease_scrollify',handleScrollify_ease);tooto.settings.page.addChangeCallback('setHeights',handleScrollify_setHeights);tooto.settings.page.addChangeCallback('overflowScroll',handleScrollify_overflowScroll);tooto.settings.page.addChangeCallback('updateHash',handleScrollify_updateHash);tooto.settings.page.addChangeCallback('scrollBars',handleScrollify_scrollBars);tooto.settings.page.addChangeCallback('touchScroll',handleScrollify_touchScroll);tooto.settings.page.addChangeCallback('enable_scrollify_nav',handleScrollify_enablenavigation);tooto.settings.page.addChangeCallback('snapscroll_nav_style',handleScrollify_navstyle);tooto.settings.page.addChangeCallback('snapscroll_nav_title_style',handleScrollify_titlestyle);tooto.settings.page.addChangeCallback('enable_scrollEffects',handleScrollEffects);tooto.settings.page.addChangeCallback('animation_effects',handleScrollEffects_animations);tooto.settings.page.addChangeCallback('remove_first_scrollEffects',handleScrollEffects_removefirst);tooto.settings.page.addChangeCallback('enable_inertiaScroll',handleInertiaScroll);tooto.settings.page.addChangeCallback('directionScroll',handleInertiaScroll_direction);tooto.settings.page.addChangeCallback('coefSpeed_inertiaScroll',handleInertiaScroll);tooto.settings.page.addChangeCallback('skew_inertiaScroll',handleInertiaScroll);tooto.settings.page.addChangeCallback('bounce_inertiaScroll',handleInertiaScroll)}}}})})(jQuery)