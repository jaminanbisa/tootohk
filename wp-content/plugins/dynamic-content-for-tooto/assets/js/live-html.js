...
function initializeAllLiveHtmlFields($scope) {
	$scope.find('.tooto-field-type-dce_live_html').each((_, w) => initializeLiveHtml(w, $scope[0]));
}

jQuery(window).on('tooto/frontend/init', function() {
	tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeAllLiveHtmlFields);
});
