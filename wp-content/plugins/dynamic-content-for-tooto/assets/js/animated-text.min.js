...
var changeText=function(){if(currentTextIndex<texts.length-1){oldTextIndex=currentTextIndex;currentTextIndex++;target.removeClass('tooto-repeater-item-'+ids[oldTextIndex]);target.html(texts[currentTextIndex]);target.addClass('tooto-repeater-item-'+ids[currentTextIndex])}else{oldTextIndex=-1;currentTextIndex=0;if(animTextRepeat>-1&&cycle>=animTextRepeat){return}
cycle++}
if(cycle===parseInt(animTextRepeat)&&currentTextIndex===texts.length-1){isLastTextCycle=!0}
animSplitText()}
var animSplitText=function(){var tl=new gsap.timeline({paused:!0});if(oldTextIndex<0){target.html(texts[0]);target.addClass('tooto-repeater-item-'+ids[0])}
