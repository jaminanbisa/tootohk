(function ($) {
    var WidgetElements_ViewsHandler = function ($scope, $) {
		...
		var elementSwiper = '.tooto-element-' + id_scope + ' .swiper-container';
		...
		if (elementSettings.navigation != 'none') {
			...
			if ( elementSettings.navigation == 'both' || elementSettings.navigation == 'arrows' ) {
				viewsSwiperOptions = $.extend(viewsSwiperOptions, {
					navigation: {
						nextEl: id_post ? '.tooto-element-' + id_scope + '[data-post-id="' + id_post + '"] .tooto-swiper-button-next' : '.tooto-swiper-button-next',
						prevEl: id_post ? '.tooto-element-' + id_scope + '[data-post-id="' + id_post + '"] .tooto-swiper-button-prev' : '.tooto-swiper-button-prev',
					},
				});
			}
			...
			if ( elementSettings.navigation == 'both' || elementSettings.navigation == 'dots' ) {
				viewsSwiperOptions = $.extend(viewsSwiperOptions, {
					pagination: {
						el: id_post ? '.tooto-element-' + id_scope + '[data-post-id="' + id_post + '"] .swiper-pagination' : '.swiper-pagination',
						type: 'bullets',
						clickable: true,
					},
				});
    ...
    $(window).on('tooto/frontend/init', function () {
        tootoFrontend.hooks.addAction('frontend/element_ready/dce-views.default', WidgetElements_ViewsHandler);
    });
})(jQuery);
