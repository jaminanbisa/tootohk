(function ($) {
    ...
    class Smooth {
        ...
        setWidth() {
            heightAdminBar = 0;
            if ($('body').is('.admin-bar') && !tootoFrontend.isEditMode()) {
                heightAdminBar = 45;
            }
    ...
    var init_Scrollify = function ( ) {
        ...
        if (settings_page.custom_class_section_sfy) {
            ...
            $customClass = 'tooto-section:not(.tooto-inner-section)';
        }

        $target_sections = '.tooto-' + currentPostId;
        if (!$target_sections)
            $target_sections = '';

        sezioni = $target_sections + '.tooto .tooto-section-wrap .' + $customClass;
        wrapperSezioni = $($target_sections + '.tooto .tooto-section-wrap');
        ...
        $.scrollify({
            ...
            afterRender: function () {
                is_scrollify = true;
                if (settings_page.enable_scrollify_nav || tootoFrontend.isEditMode()) {
    ...
    var init_InertiaScroll = function () {
        ...
        if ($('.tooto-template-canvas').length) {
            main = document.querySelector($target_sections);
            mainWrap = document.querySelector($target_sections + '.tooto .tooto-section-wrap');
        } else {
    ...
    $(window).on('tooto/frontend/init', function () {
       ...
       if (typeof tootoFrontendConfig.settings.page !== 'undefined') {
           settings_page = tootoFrontendConfig.settings.page;
           currentPostId = tootoFrontendConfig.post.id;
           ...
           if (tootoFrontend.isEditMode()) {
               if (tooto) {
                   settings_page = tooto.settings.page.model.attributes;

                   tooto.settings.page.addChangeCallback('enable_dceScrolling', handlescroll_viewport);

                   // Scrollify
                   tooto.settings.page.addChangeCallback('enable_scrollify', handleScrollify);
                   tooto.settings.page.addChangeCallback('scrollSpeed', handleScrollify_speed);
                   tooto.settings.page.addChangeCallback('offset', handleScrollify_offset);
                   tooto.settings.page.addChangeCallback('ease_scrollify', handleScrollify_ease);
                   tooto.settings.page.addChangeCallback('setHeights', handleScrollify_setHeights);
                   tooto.settings.page.addChangeCallback('overflowScroll', handleScrollify_overflowScroll);
                   tooto.settings.page.addChangeCallback('updateHash', handleScrollify_updateHash);
                   tooto.settings.page.addChangeCallback('scrollBars', handleScrollify_scrollBars);
                   tooto.settings.page.addChangeCallback('touchScroll', handleScrollify_touchScroll);
                   tooto.settings.page.addChangeCallback('enable_scrollify_nav', handleScrollify_enablenavigation);
                   tooto.settings.page.addChangeCallback('snapscroll_nav_style', handleScrollify_navstyle);
                   tooto.settings.page.addChangeCallback('snapscroll_nav_title_style', handleScrollify_titlestyle);

                   // ScrollEffects
                   tooto.settings.page.addChangeCallback('enable_scrollEffects', handleScrollEffects);
                   tooto.settings.page.addChangeCallback('animation_effects', handleScrollEffects_animations);
                   tooto.settings.page.addChangeCallback('remove_first_scrollEffects', handleScrollEffects_removefirst);

                   // InertiaScroll
                   tooto.settings.page.addChangeCallback('enable_inertiaScroll', handleInertiaScroll);
                   tooto.settings.page.addChangeCallback('directionScroll', handleInertiaScroll_direction);

                   tooto.settings.page.addChangeCallback('coefSpeed_inertiaScroll', handleInertiaScroll);
                   tooto.settings.page.addChangeCallback('skew_inertiaScroll', handleInertiaScroll);
                   tooto.settings.page.addChangeCallback('bounce_inertiaScroll', handleInertiaScroll);
               }
           }
       }
   });
