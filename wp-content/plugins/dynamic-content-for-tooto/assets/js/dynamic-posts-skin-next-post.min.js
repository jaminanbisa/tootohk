...
var template_id=elementSettings[DCE_dynposts_skinPrefix+'nextpost_content_template_id'];eval('var list_posts = dce_listPosts_'+id_scope+';');var list_post_associate=[];var totalPosts=list_posts.length;$scope.removeClass('tooto-widget-empty');var introSection,introTitle,introContent,introSectionHeight=480,scrtop=0,scaleSpeed=0.3,bgsScaleSpeed=10,translateSpeed=200,opacitySpeed=1;var initEffectScroll=function(){triggerAnimation()};function triggerAnimation(){$(window).on('scroll',function(){window.requestAnimationFrame(animateIntro)})}
...
var index=(type=='next')?this.nextPostIndex(this.currentPostIndex):this.currentPostIndex;this.getPost(index,function(d){self.contentizeElement(self['$'+type],d);callback&&callback()})};ArticleAnimator.contentizeElement=function($el,d){$el.find('.dce-big-image').css({backgroundImage:"url("+d.image+")"});$el.find('.dce-title').html(d.title);$el.find('.dce-description').html(d.title);if(use_template){jQuery.ajax({url:dceAjaxPath.ajaxurl,dataType:"html",type:'POST',data:{'action':'modale_action','post_href':d.link,'template_id':template_id},error:function(){erroreModale()},success:function(data,status,xhr){var $result=jQuery(data).filter('.content-p');$result.find('.titolo-nativo').remove();$el.find('.dce-content .dce-text').html($result);var element_el=$result.find('.tooto-element');element_el.each(function(i){var el=jQuery(this).data('widget_type');tootoFrontend.elementsHandler.runReadyTrigger(jQuery(this))})},})}else{$el.find('.dce-content .dce-text').html(d.content.rendered)}
...
$body=$scope.find('.dce-nextpost-wrapper');$window=$(window);$html=$(document.documentElement);ArticleAnimator.load();initEffectScroll()};jQuery(window).on('tooto/frontend/init',function(){tootoFrontend.hooks.addAction('frontend/element_ready/dce-dynamicposts-v2.nextpost',Widget_DCE_Dynamicposts_nextpost_Handler)})