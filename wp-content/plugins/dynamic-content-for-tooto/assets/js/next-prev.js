(function ($) {
    var WidgetElements_NextPrevHandler = function ($scope, $) {
        if (typeof $('#main').ajaxify === "function") {
            $('#main').ajaxify({
                ...
                selector: ".tooto-widget-dyncontel-post-nextprev a:not(.no-ajaxy)", //Selector for elements to ajaxify - without being swapped - e.g. a selection of links
    ...
    $(window).on('pronto.render', function (event, eventInfo) {
        ...
        if (typeof eventInfo.delegateTarget !== 'undefined') {
            var element_el = $(eventInfo.delegateTarget.innerHTML).find('.tooto-element');
        ...
        $(window).trigger('tooto/frontend/init');
        $(window).trigger('ready');

    });
    // Make sure you run this code under tooto..
    $(window).on('tooto/frontend/init', function () {
        tootoFrontend.hooks.addAction('frontend/element_ready/dyncontel-post-nextprev.default', WidgetElements_NextPrevHandler);
    });
})(jQuery);
