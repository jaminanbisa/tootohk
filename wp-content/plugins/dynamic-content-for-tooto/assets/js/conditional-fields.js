...
const getAllFields = ($form, fieldIds) => {
	...
	for (const id of fieldIds) {
		const $group = $form.find('.tooto-field-group-' + id);
		if ( $group.length ) {
...
const initializeStepsJumping = ($form) => {
	...
	let $steps = $form.find('.tooto-field-type-step');
	if ($steps.length < 3) {
	...
	for (const step of $steps) {
		...
		const $next = $step.find('.tooto-field-type-next button');
		const $previous = $step.find('.tooto-field-type-previous button');
		steps.push({
			nextButton: $next,
...
const getOnFormChange = ({fields, field_conditions, submit_conditions, $form, lang}) => {
	const $submitButton = $form.find('.tooto-field-type-submit');
...
function initializeConditionalFields($form) {
	const $outWrapper = $form.find('.tooto-form-fields-wrapper');
...
jQuery(window).on('tooto/frontend/init', function() {
	tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeConditionalFields);
	// We delay adding the hook so it is run after the steps ares initialized:
	tootoFrontend.hooks.addAction('frontend/element_ready/global', () => {
		tootoFrontend.hooks.addAction('frontend/element_ready/form.default', initializeStepsJumping);
	});
});
