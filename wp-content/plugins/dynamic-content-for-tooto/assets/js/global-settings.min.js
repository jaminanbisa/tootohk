...
function handleAnimsition(newValue){$('body').toggleClass('dce-smoothtransition');if(newValue){if(!is_animsition){settings_global=tooto.settings.dynamicooo.model.attributes}
...
function handleHeaderTracker(newValue){settings_global=tooto.settings.dynamicooo.model.attributes;if(newValue){if(!$('body.dce-trackerheader').length)
...
$(window).on('tooto/frontend/init',function(){if(tootoFrontend.isEditMode()){if(tooto.settings.dynamicooo){tooto.settings.dynamicooo.addChangeCallback('enable_smoothtransition',handleAnimsition);tooto.settings.dynamicooo.addChangeCallback('dce_smoothtransition_class_controller',handleAnimsition);tooto.settings.dynamicooo.addChangeCallback('smoothtransition_enable_overlay',handleOverlay);tooto.settings.dynamicooo.addChangeCallback('smoothtransition_loading_mode',handleLoadingMode);tooto.settings.dynamicooo.addChangeCallback('smoothtransition_loading_style',handleLoadingStyle);tooto.settings.dynamicooo.addChangeCallback('enable_trackerheader',handleHeaderTracker);tooto.settings.dynamicooo.addChangeCallback('dce_trackerheader_class_controller',handleHeaderTracker);tooto.settings.dynamicooo.addChangeCallback('trackerheader_overlay',handleTrackerHeader_Overlay)}}});$(function(){if(typeof tootoFrontendConfig!=="undefined"){settings_global=tootoFrontendConfig.settings.dynamicooo}else{settings_global=dceGlobalSettings}
if(settings_global){var responsive_smoothtransition=settings_global.responsive_smoothtransition||['desktop','tablet','mobile'];var enableSmoothtransition=settings_global.enable_smoothtransition||'';var smoothtransitionClassController=settings_global.dce_smoothtransition_class_controller||'';var responsive_trackerheader=settings_global.responsive_trackerheader||['desktop','tablet','mobile'];var enableTrackerHeader=settings_global.enable_trackerheader||'';var trackerheaderClassController=settings_global.dce_trackerheader_class_controller||'';var deviceMode=$('body').attr('data-tooto-device-mode')||'desktop';if(enableSmoothtransition&&$.inArray(deviceMode,responsive_smoothtransition)>=0&&smoothtransitionClassController)
