...
eval(tweenString);eval(tweenSVG);is_running=!0;if(run=='paused'&&tootoFrontend.isEditMode()){ferma()}
...
$('#dce-svg-'+id_scope).attr('data-morphid',step)};var waypointOptions={offset:'100%',triggerOnce:!1};tootoFrontend.waypoint($($el),runAnim,waypointOptions)}};var mouseenterFn=function(){step=1;$('#dce-svg-'+id_scope).attr('data-morphid',step);if(!is_running)
moveToStep(step)};var mouseleaveFn=function(){step=0;$('#dce-svg-'+id_scope).attr('data-morphid',step);if(!is_running)
moveToStep(step)};if(!tootoFrontend.isEditMode()&&contentElemsTotal>1&&elementSettings.svg_trigger=='animation'){$('#dce-svg-'+id_scope).attr('data-run','running')}
if(tootoFrontend.isEditMode()){if(transitionTl)transitionTl.kill($(dceshape));if(transitionTlpos)transitionTlpos.kill($(dceshape_svg));$('.tooto-element[data-id='+id_scope+']').off('mouseenter','svg');$('.tooto-element[data-id='+id_scope+']').off('mouseleave','svg');$('.tooto-element[data-id='+id_scope+']').off('touchstart','svg');$('.tooto-element[data-id='+id_scope+']').off('touchend','svg')}
setTimeout(function(){get_data_anim();get_data_shape();if(elementSettings.svg_trigger=='animation'){createTween();if(tootoFrontend.isEditMode()&&contentElemsTotal>1)playShapeEl()}else if(elementSettings.svg_trigger=='rollover'){if(transitionTl)transitionTl.kill($(dceshape));if(transitionTlpos)transitionTlpos.kill($(dceshape_svg));transitionTl=new gsap.timeline();transitionTlpos=new gsap.timeline();$('.tooto-element[data-id='+id_scope+']').on('mouseenter','svg',mouseenterFn);$('.tooto-element[data-id='+id_scope+']').on('mouseleave','svg',mouseleaveFn);$('.tooto-element[data-id='+id_scope+']').on('touchstart','svg',mouseenterFn);$('.tooto-element[data-id='+id_scope+']').on('touchend','svg',mouseleaveFn)}else if(elementSettings.svg_trigger=='scroll'){if(one_by_one){if(transitionTl)transitionTl.kill($(dceshape));if(transitionTlpos)transitionTlpos.kill($(dceshape_svg));transitionTl=new gsap.timeline();transitionTlpos=new gsap.timeline()}else{if(playpause_control=='paused'){ferma()}else{createTween()}
if(tootoFrontend.isEditMode()&&contentElemsTotal>1)playShapeEl()}
active_scrollAnalysi('#dce-svg-'+id_scope)}},100)};$(window).on('tooto/frontend/init',function(){tootoFrontend.hooks.addAction('frontend/element_ready/dyncontel-svgmorphing.default',WidgetElements_SvgMorphHandler)})})(jQuery)