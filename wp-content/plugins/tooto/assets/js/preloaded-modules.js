...
(self["webpackChunktooto"] = self["webpackChunktooto"] || []).push([["preloaded-modules"],{
...
exports["default"] = void 0;
class baseTabs extends tootoModules.frontend.handlers.Base {
  getDefaultSettings() {
    return {
      selectors: {
        tablist: '[role="tablist"]',
        tabTitle: '.tooto-tab-title',
        tabContent: '.tooto-tab-content'
      },
      classes: {
        active: 'tooto-active'
      },
      ...
      autoExpand: true,
      keyDirection: {
        ArrowLeft: tootoFrontendConfig.is_rtl ? 1 : -1,
        ArrowUp: -1,
        ArrowRight: tootoFrontendConfig.is_rtl ? -1 : 1,
        ArrowDown: 1
      }
    };
  }
  ...
  activateTab(tabIndex) {
    ...
    $requestedContent[settings.showTabFn](animationDuration, () => tootoFrontend.elements.$window.trigger('tooto-pro/motion-fx/recalc'));
...
exports["default"] = void 0;
class Video extends tootoModules.frontend.handlers.Base {
  getDefaultSettings() {
    return {
      selectors: {
        imageOverlay: '.tooto-custom-embed-image-overlay',
        video: '.tooto-video',
        videoIframe: '.tooto-video-iframe',
        playIcon: '.tooto-custom-embed-play'
      }
    };
  }
  getDefaultElements() {
  ...
  onInit() {
    super.onInit();
    const elementSettings = this.getElementSettings();
    if (tootoFrontend.utils[elementSettings.video_type]) {
      this.apiProvider = tootoFrontend.utils[elementSettings.video_type];
    } else {
      this.apiProvider = tootoFrontend.utils.baseVideoLoader;
    }
...
var _nestedTabs = _interopRequireDefault(__webpack_require__(/*! tooto/modules/nested-tabs/assets/js/frontend/handlers/nested-tabs */ "../modules/nested-tabs/assets/js/frontend/handlers/nested-tabs.js"));
var _lightbox = _interopRequireDefault(__webpack_require__(/*! tooto-frontend/utils/lightbox/lightbox */ "../assets/dev/js/frontend/utils/lightbox/lightbox.js"));
tootoFrontend.elements.$window.on('tooto/frontend/init', () => {
  tootoFrontend.elementsHandler.elementsHandlers = {
  ...
  tootoFrontend.on('components:init', () => {
    ...
    delete tootoFrontend.utils.lightbox;
    tootoFrontend.utils.lightbox = new _lightbox.default();
  });
});
...
var _eIcons = __webpack_require__(/*! @tooto/e-icons */ "../assets/dev/js/frontend/utils/icons/e-icons.js");
module.exports = tootoModules.ViewModule.extend({
  oldAspectRatio: null,
  oldAnimation: null,
  swiper: null,
  player: null,
  isFontIconSvgExperiment: tootoFrontend.config.experimentalFeatures.e_font_icon_svg,
  getDefaultSettings() {
    return {
      classes: {
        aspectRatio: 'tooto-aspect-ratio-%s',
        item: 'tooto-lightbox-item',
        image: 'tooto-lightbox-image',
        videoContainer: 'tooto-video-container',
        videoWrapper: 'tooto-fit-aspect-ratio',
        playButton: 'tooto-custom-embed-play',
        playButtonIcon: 'fa',
        playing: 'tooto-playing',
        hidden: 'tooto-hidden',
        invisible: 'tooto-invisible',
        preventClose: 'tooto-lightbox-prevent-close',
        slideshow: {
          container: tootoFrontend.config.swiperClass,
          slidesWrapper: 'swiper-wrapper',
          prevButton: 'tooto-swiper-button tooto-swiper-button-prev',
          nextButton: 'tooto-swiper-button tooto-swiper-button-next',
          prevButtonIcon: 'eicon-chevron-left',
          nextButtonIcon: 'eicon-chevron-right',
          slide: 'swiper-slide',
          header: 'tooto-slideshow__header',
          footer: 'tooto-slideshow__footer',
          title: 'tooto-slideshow__title',
          description: 'tooto-slideshow__description',
          counter: 'tooto-slideshow__counter',
          iconExpand: 'eicon-frame-expand',
          iconShrink: 'eicon-frame-minimize',
          iconZoomIn: 'eicon-zoom-in-bold',
          iconZoomOut: 'eicon-zoom-out-bold',
          iconShare: 'eicon-share-arrow',
          shareMenu: 'tooto-slideshow__share-menu',
          shareLinks: 'tooto-slideshow__share-links',
          hideUiVisibility: 'tooto-slideshow--ui-hidden',
          shareMode: 'tooto-slideshow--share-mode',
          fullscreenMode: 'tooto-slideshow--fullscreen-mode',
          zoomMode: 'tooto-slideshow--zoom-mode'
        }
      },
      selectors: {
        image: '.tooto-lightbox-image',
  ...
  initModal() {
    ...
    const modal = module.exports.modal = tootoFrontend.getDialogsManager().createWidget('lightbox', {
      className: 'tooto-lightbox',
      closeButton: true,
      closeButtonOptions: {
        ...closeIcon,
        attributes: {
          tabindex: 0,
          role: 'button',
          'aria-label': tootoFrontend.config.i18n.close + ' (Esc)'
        }
      },
      selectors: {
  ...
  createLightbox(element) {
    let lightboxData = {};
    if (element.dataset.tootoLightbox) {
      lightboxData = JSON.parse(element.dataset.tootoLightbox);
    }
    if (lightboxData.type && 'slideshow' !== lightboxData.type) {
      this.showModal(lightboxData);
      return;
    }
    if (!element.dataset.tootoLightboxSlideshow) {
      ...
      this.showModal({
        ...
        title: element.dataset.tootoLightboxTitle,
        description: element.dataset.tootoLightboxDescription,
        modalOptions: {
          id: 'tooto-lightbox-slideshow-' + slideshowID
        }
      });
      return;
    }
    const initialSlideURL = element.dataset.tootoLightboxVideo || element.href;
    this.openSlideshow(element.dataset.tootoLightboxSlideshow, initialSlideURL);
  },
  setHTMLContent(html) {
    if (window.tootoCommon) {
      tootoDevTools.deprecation.deprecated('tootoFrontend.utils.lightbox.setHTMLContent', '3.1.4');
    }
  ...
  setVideoContent(options) {
    ...
    if ('hosted' === options.videoType) {
      ...
      if (-1 !== options.url.indexOf('vimeo.com')) {
        apiProvider = tootoFrontend.utils.vimeo;
      } else if (options.url.match(/^(?:https?:\/\/)?(?:www\.)?(?:m\.)?(?:youtu\.be\/|youtube\.com|youtube-nocookie\.com)/)) {
        apiProvider = tootoFrontend.utils.youtube;
      } else {
        return;
      }
      $videoElement = $('<iframe>', {
    ...
    modal.onHide = function () {
      ...
      this.$buttons = jQuery();
      this.focusedButton = null;
      modal.getElements('message').removeClass('tooto-fit-aspect-ratio');
    };
  },
  getShareLinks() {
    const {
        i18n
      } = tootoFrontend.config,
      socialNetworks = {
        facebook: {
      ...
      videoUrl = $activeSlide.data('tooto-slideshow-video');
    let itemUrl;
    if (videoUrl) {
      itemUrl = videoUrl;
    } else {
  ...
  getSlideshowHeader() {
    const {
      ...
      $ = jQuery,
      showCounter = 'yes' === tootoFrontend.getKitSettings('lightbox_enable_counter'),
      showFullscreen = 'yes' === tootoFrontend.getKitSettings('lightbox_enable_fullscreen'),
      showZoom = 'yes' === tootoFrontend.getKitSettings('lightbox_enable_zoom'),
      showShare = 'yes' === tootoFrontend.getKitSettings('lightbox_enable_share'),
      classes = this.getSettings('classes'),
  ...
  setSlideshowContent(options) {
    const {
      ...
      hasTitle = '' !== tootoFrontend.getKitSettings('lightbox_title_src'),
      hasDescription = '' !== tootoFrontend.getKitSettings('lightbox_description_src'),
    ...
    options.slides.forEach(slide => {
      ...
      if (slide.video) {
        $slide.attr('data-tooto-slideshow-video', slide.video);
  ...
  updateFooterText() {
    ...
    const classes = this.getSettings('classes'),
      $activeSlide = this.getSlide('active'),
      $image = $activeSlide.find('.tooto-lightbox-image'),
  ...
  prepareYTVideo(YT, videoID, $videoContainer, $videoWrapper, $playIcon) {
    ...
    $videoContainer.addClass('tooto-loading' + ' ' + classes.invisible);
      ...
      events: {
        ...
        onStateChange: event => {
          if (event.data === startStateCode) {
            $videoContainer.removeClass('tooto-loading' + ' ' + classes.invisible);
          }
        }
      },
      playerVars: {
  ...
  setEntranceAnimation(animation) {
    animation = animation || tootoFrontend.getCurrentDeviceSetting(this.getSettings('modalOptions'), 'entranceAnimation');
  ...
  openSlideshow(slideshowID, initialSlideURL) {
    const $allSlideshowLinks = jQuery(this.getSettings('selectors.links')).filter((index, element) => {
      ...
      return slideshowID === element.dataset.tootoLightboxSlideshow && !$element.parent('.swiper-slide-duplicate').length && !$element.parents('.slick-cloned').length;
    });
    const slides = [];
    let initialSlideIndex = 0;
    $allSlideshowLinks.each(function () {
      const slideVideo = this.dataset.tootoLightboxVideo;
      let slideIndex = this.dataset.tootoLightboxIndex;
      ...
      const slideData = {
        image: this.href,
        index: slideIndex,
        title: this.dataset.tootoLightboxTitle,
        description: this.dataset.tootoLightboxDescription,
