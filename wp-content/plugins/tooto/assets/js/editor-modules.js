...
var Container = /*#__PURE__*/function (_ArgsObject) {
  ...
  function Container(args) {
    ...
    if (!_this.document) {
      _this.document = tooto.documents.getCurrent();
    }
  ...
  (0, _createClass2.default)(Container, [{
    ...
    value: function validateArgs(args) {
      ...
      if (false !== args.parent) {
        this.requireArgumentInstance('parent', tootoModules.editor.Container, args);
      }
    }
    ...
    value: function handleRepeaterChildren() {
      ...
      Object.values(this.controls).forEach(function (control) {
        ...
        _this5.repeaters[control.name] = new tootoModules.editor.Container({
      ...
      if (['widget', 'document'].includes(this.type)) {
        ...
        if (!this.model.get('supportRepeaterChildren') && 1 === repeaters.length) {
          Object.defineProperty(this, 'children', {
            get: function get() {
              tootoDevTools.deprecation.deprecated('children', '3.0.0', 'container.repeaters[ repeaterName ].children');
    ...
    value: function addRepeaterItem(repeaterName, rowSettingsModel, index) {
      ...
      if (!rowId) {
        rowId = 'bc-' + tootoCommon.helpers.getUniqueId();
        rowSettingsModel.set('_id', rowId);
      }
      this.repeaters[repeaterName].children.splice(index, 0, new tootoModules.editor.Container({
        ...
        label: this.label + ' ' + __('Item', 'tooto'),
    ...
    value: function findChildrenRecursive(callback) {
      tootoDevTools.deprecation.deprecated('container.findChildrenRecursive( callback )', '3.5.0', 'container.children.findRecursive( callback )');
      return this.children.findRecursive(callback);
    }
  }, {
    key: "forEachChildrenRecursive",
    value: function forEachChildrenRecursive(callback) {
      tootoDevTools.deprecation.deprecated('container.forEachChildrenRecursive( callback )', '3.5.0', 'container.children.forEachRecursive( callback )');
    ...
    value: function isEditable() {
      return 'edit' === tooto.channels.dataEditMode.request('activeMode') && 'open' === this.document.editor.status;
    }
  }, {
    key: "isDesignable",
    value: function isDesignable() {
      return tooto.userCan('design') && this.isEditable();
    }
    ...
    value: function getGlobalValue(name) {
      ...
      if (control.groupType) {
        ...
        var responsivePrefixRegex = tooto.breakpoints.getActiveMatchRegex();
        var propertyName = control.name.replace(control.groupPrefix, '').replace(responsivePrefixRegex, '');
        if (!data.value[tooto.config.kit_config.typography_prefix + propertyName]) {
          return;
        }
        propertyName = propertyName.replace('_', '-');
        value = "var( --e-global-".concat(control.groupType, "-").concat(id, "-").concat(propertyName, " )");
        if (tooto.config.ui.defaultGenericFonts && control.groupPrefix + 'font_family' === control.name) {
          value += ", ".concat(tooto.config.ui.defaultGenericFonts);
        }
      } else {
    ...
    value: function getGlobalDefault(controlName) {
      ...
      if (controlGlobalArgs !== null && controlGlobalArgs !== void 0 && controlGlobalArgs.default) {
        ...
        if (!tooto.config.globals.defaults_enabled[controlType]) {
          return '';
        }
...
var Panel = /*#__PURE__*/function () {
  ...
  (0, _createClass2.default)(Panel, [{
    ...
    value: function getControlView(name) {
      var editor = tooto.getPanelView().getCurrentPageView();
...
BaseSettingsModel = Backbone.Model.extend({
  ...
  initialize: function initialize(data, options) {
    ...
    self.controls = tooto.mergeControlsSettings(options.controls);
    self.validators = {};
    if (!self.controls) {
      return;
    }
    ...
    _.each(self.controls, function (control) {
      ...
      if ('object' === (0, _typeof2.default)(control.default)) {
        defaults[controlName] = tootoCommon.helpers.cloneObject(control.default);
      } else {
      ...
      if (undefined !== attrs[controlName] && isMultipleControl && !_.isObject(attrs[controlName]) && !hasDynamicSettings) {
        tootoCommon.debug.addCustomError(new TypeError('An invalid argument supplied as multiple control value'), 'InvalidElementData', 'Element `' + (self.get('widgetType') || self.get('elType')) + '` got <' + attrs[controlName] + '> as `' + controlName + '` value. Expected array or object.');
  ...
  convertRepeaterValueToCollection: function convertRepeaterValueToCollection(attrs, repeaterControl) {
    return new Backbone.Collection(attrs[repeaterControl.name], {
      model: function model(attributes, options) {
        ...
        if (!attributes._id) {
          attributes._id = tootoCommon.helpers.getUniqueId();
        }
  ...
  getStyleControls: function getStyleControls(controls, attributes) {
    var self = this;
    controls = tootoCommon.helpers.cloneObject(self.getActiveControls(controls, attributes));
    var styleControls = [];
    jQuery.each(controls, function () {
      var _control$dynamic;
      var control = this,
        controlDefaultSettings = tooto.config.controls[control.type];
  ...
  getActiveControls: function getActiveControls(controls, attributes) {
    ...
    jQuery.each(controls, function (controlKey, control) {
      if (tooto.helpers.isActiveControl(control, attributes, controls)) {
  ...
  clone: function clone() {
    return new BaseSettingsModel(tootoCommon.helpers.cloneObject(this.attributes), tootoCommon.helpers.cloneObject(this.options));
  },
  ...
  parseDynamicSettings: function parseDynamicSettings(settings, options, controls) {
    var self = this;
    settings = tootoCommon.helpers.cloneObject(settings || self.attributes);
    options = options || {};
    controls = controls || this.controls;
    jQuery.each(controls, function () {
      var control = this,
      ...
      if (undefined === dynamicSettings) {
        dynamicSettings = tooto.config.controls[control.type].dynamic;
      }
      ...
      var dynamicValue;
      try {
        dynamicValue = tooto.dynamicTags.parseTagsText(valueToParse, dynamicSettings, tooto.dynamicTags.getTagDataContent);
      } catch (error) {
        if (tooto.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR !== error.message) {
          throw error;
        }
        dynamicValue = '';
        ...
        tooto.dynamicTags.refreshCacheFromServer(function () {
  ...
  parseGlobalSettings: function parseGlobalSettings(settings, controls) {
    var _this = this;
    settings = tootoCommon.helpers.cloneObject(settings);
    controls = controls || this.controls;
    jQuery.each(controls, function (index, control) {
      ...
      if (undefined === globalSettings) {
        globalSettings = tooto.config.controls[control.type].global;
      }
  ...
  toJSON: function toJSON(options) {
    ...
    return tootoCommon.helpers.cloneObject(data);
  }
});
...
InnerTabsBehavior = Marionette.Behavior.extend({
  ...
  onChildviewControlTabClicked: function onChildviewControlTabClicked(childView) {
    ...
    tooto.getPanelView().updateScrollbar();
  }
});
...
var _default = /*#__PURE__*/function (_tootoModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initDialog() {
      ...
      this.getDialog = function () {
        if (!dialog) {
          ...
          dialog = tootoCommon.dialogsManager.createWidget(settings.dialogType, settings.dialogOptions);
    ...
    value: function setViewed() {
      ...
      tootoCommon.ajax.addRequest('introduction_viewed', {
        data: {
...
var EditorModule = tootoModules.Module.extend({
  onInit: function onInit() {
    var _this = this;
    var $window = jQuery(window);
    $window.on('tooto:init-components', this.ontootoInitComponents.bind(this));
    $window.on('tooto:loaded', function () {
      _this.ontootoLoaded();
      tooto.on('document:loaded', _this.onDocumentLoaded.bind(_this));
    });
    $window.on('tooto:init', this.ontootoReady);
  },
  ...
  ontootoReady: function ontootoReady() {
    this.ontootoInit();
    tooto.on('frontend:init', this.ontootoFrontendInit.bind(this)).on('preview:loaded', this.ontootoPreviewLoaded.bind(this));
  }
});
EditorModule.prototype.ontootoLoaded = function () {};
EditorModule.prototype.ontootoInit = function () {};
EditorModule.prototype.ontootoPreviewLoaded = function () {};
EditorModule.prototype.onDocumentLoaded = function () {};
EditorModule.prototype.ontootoFrontendInit = function () {};
EditorModule.prototype.ontootoInitComponents = function () {};
  ...
  (0, _createClass2.default)(ControlsPopover, [{
    ...
    value: function createPopoverHeader() {
      ...
      var $popoverToggleControl = this.$popover.prev(),
        ...
        $resetLabel = $popoverToggleControl.find('.tooto-control-popover-toggle-reset-label');
      this.$popoverHeader = jQuery('<div>', {
        class: 'e-group-control-header'
      }).html('<span>' + __('Typography', 'tooto') + '</span>');
    ...
    value: function createAddButton() {
      ...
      this.$addButton.tipsy({
        title: function title() {
          return __('Create New Global Font', 'tooto');
        },
...
ControlsStack = Marionette.CompositeView.extend({
  classes: {
    popover: 'tooto-controls-popover'
  },
  activeTab: null,
  activeSection: null,
  className: function className() {
    return 'tooto-controls-stack';
  },
  templateHelpers: function templateHelpers() {
    return {
      elementData: tooto.getElementData(this.model)
    };
  },
  ...
  ui: function ui() {
    return {
      tabs: '.tooto-panel-navigation-tab',
      reloadButton: '.tooto-update-preview-button'
    };
  },
  ...
  behaviors: {
    HandleInnerTabs: {
      behaviorClass: __webpack_require__(/*! tooto-behaviors/inner-tabs */ "../assets/dev/js/editor/elements/views/behaviors/inner-tabs.js")
    }
  },
  initialize: function initialize(options) {
    ...
    this.listenTo(tooto.channels.deviceMode, 'change', this.onDeviceModeChange);
  },
  initCollection: function initCollection() {
    this.collection = new Backbone.Collection(_.values(tooto.mergeControlsSettings(this.getOption('controls'))));
  },
  ...
  getChildView: function getChildView(item) {
    ...
    return tooto.getControlView(controlType);
  },
  getNamespaceArray: function getNamespaceArray() {
    return [tooto.getPanelView().getCurrentPageName()];
  },
  openActiveSection: function openActiveSection() {
    ...
    if (activeSectionView[0]) {
      activeSectionView[0].$el.addClass('tooto-open');
      var eventNamespace = this.getNamespaceArray();
      eventNamespace.push(activeSection, 'activated');
      tooto.channels.editor.trigger(eventNamespace.join(':'), this);
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    if ('desktop' === tooto.channels.deviceMode.request('currentMode')) {
      this.$el.removeClass('tooto-responsive-switchers-open');
    }
  },
  onChildviewControlSectionClicked: function onChildviewControlSectionClicked(childView) {
    var isSectionOpen = childView.$el.hasClass('tooto-open');
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      this.$el.toggleClass('tooto-responsive-switchers-open');
    }
  }
}, {
