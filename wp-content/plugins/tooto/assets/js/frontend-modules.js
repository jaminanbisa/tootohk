...
(self["webpackChunktooto"] = self["webpackChunktooto"] || []).push([["frontend-modules"],{
...
class _default extends tootoModules.ViewModule {
  getDefaultSettings() {
    return {
      selectors: {
        elements: '.tooto-element',
        nestedDocumentElements: '.tooto .tooto-element'
      },
      classes: {
        editMode: 'tooto-edit-mode'
      }
    };
  }
  getDefaultElements() {
  ...
  getDocumentSettings(setting) {
    ...
    if (this.isEdit) {
      elementSettings = {};
      const settings = tooto.settings.page.model;
      jQuery.each(settings.getActiveControls(), controlKey => {
        elementSettings[controlKey] = settings.attributes[controlKey];
      });
    } else {
      elementSettings = this.$element.data('tooto-settings') || {};
    }
    return this.getItems(elementSettings, setting);
  }
  runElementsHandlers() {
    this.elements.$elements.each((index, element) => setTimeout(() => tootoFrontend.elementsHandler.runReadyTrigger(element)));
  }
  onInit() {
    ...
    if (this.isEdit) {
      tooto.on('document:loaded', () => {
        tooto.settings.page.model.on('change', this.onSettingsChange.bind(this));
      });
    } else {
...
module.exports = tootoModules.ViewModule.extend({
  ...
  __construct(settings) {
    ...
    this.isEdit = this.$element.hasClass('tooto-element-edit-mode');
    if (this.isEdit) {
  ...
  findElement(selector) {
    ...
    return $mainElement.find(selector).filter(function () {
      // Start `closest` from parent since self can be `.tooto-element`.
      return jQuery(this).parent().closest('.tooto-element').is($mainElement);
    });
  },
  ...
  initEditorListeners() {
    ...
    ['page'].forEach(function (settingsType) {
      ...
      if (self[listenerMethodName]) {
        self.editorListeners.push({
          event: 'change',
          to: tooto.settings[settingsType].model,
          callback(model) {
  ...
  addEditorListeners() {
    ...
    this.getEditorListeners().forEach(function (listener) {
      tootoFrontend.addListenerOnce(uniqueHandlerID, listener.event, listener.callback, listener.to);
    });
  },
  removeEditorListeners() {
    var uniqueHandlerID = this.getUniqueHandlerID();
    this.getEditorListeners().forEach(function (listener) {
      tootoFrontend.removeListeners(uniqueHandlerID, listener.event, null, listener.to);
    });
  },
  getElementType() {
  ...
  getElementSettings(setting) {
    ...
    if (this.isEdit && modelCID) {
      const settings = tootoFrontend.config.elements.data[modelCID],
        attributes = settings.attributes;
      let type = attributes.widgetType || attributes.elType;
      if (attributes.isInner) {
        type = 'inner-' + type;
      }
      let settingsKeys = tootoFrontend.config.elements.keys[type];
      if (!settingsKeys) {
  ...
  getEditSettings(setting) {
    var attributes = {};
    if (this.isEdit) {
      attributes = tootoFrontend.config.elements.editSettings[this.getModelCID()].attributes;
    }
    return this.getItems(attributes, setting);
  },
  getCurrentDeviceSetting(settingKey) {
    return tootoFrontend.getCurrentDeviceSetting(this.getElementSettings(), settingKey);
  },
  onInit() {
...
module.exports = tootoModules.ViewModule.extend({
  getDefaultSettings() {
    return {
      element: null,
      direction: tootoFrontend.config.is_rtl ? 'right' : 'left',
      selectors: {
        container: window
      }
    };
...
class NestedTabs extends _base.default {
  ...
  activateTab(tabIndex) {
    ...
    $requestedContent[settings.showTabFn](animationDuration, () => {
      tootoFrontend.elements.$window.trigger('tooto-pro/motion-fx/recalc');
      tootoFrontend.elements.$window.trigger('tooto/nested-tabs/activate', $requestedContent);
    });
  ...
  bindEvents() {
    ...
    tootoFrontend.elements.$window.on('tooto/nested-tabs/activate', this.reInitSwipers);
  }
  ...
  reInitSwipers(event, content) {
    const swiperElements = content.querySelectorAll(`.${tootoFrontend.config.swiperClass}`);
  ...
  changeActiveTab(tabIndex) {
    ...
    if (fromUser && this.isEdit) {
      return window.top.$e.run('document/repeater/select', {
        container: tooto.getContainer(this.$element.attr('data-id')),
  ...
  createMobileTabs(args) {
    ...
    if (tootoFrontend.isEditMode()) {
      ...
      if ($removed.length) {
        return tootoModules.ViewModule.prototype.onInit.apply(this, args);
      }
    }
  }
}
