...
"use strict";
(self["webpackChunktooto"] = self["webpackChunktooto"] || []).push([["text-path"],{
...
class TextPathHandler extends tootoModules.frontend.handlers.Base {
  getDefaultSettings() {
    return {
      selectors: {
  ...
  setText(newText) {
    ...
    if (this.shouldReverseText()) {
      ...
      clone.id += '-clone';
      clone.classList.add('tooto-hidden');
  ...
  isRTL() {
    ...
    let isRTL = tootoFrontend.config.is_rtl;
    if (direction) {
      isRTL = 'rtl' === direction;
    }
    return isRTL;
  ...
  shouldReverseText() {
    ...
    const isFirefox = tootoFrontend.utils.environment.firefox;
    if (isFirefox) {
      return false;
    }
    const isChromium = tootoFrontend.utils.environment.blink;
    if (isChromium) {
