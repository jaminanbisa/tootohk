...
/* global tootoGutenbergSettings */
(function ($) {
  'use strict';

  var tootoGutenbergApp = {
    cacheElements: function cacheElements() {
      var self = this;
      self.istootoMode = tootoGutenbergSettings.istootoMode;
      self.cache = {};
      self.cache.$gutenberg = $('#editor');
      self.cache.$switchMode = $($('#tooto-gutenberg-button-switch-mode').html());
      self.cache.$switchModeButton = self.cache.$switchMode.find('#tooto-switch-mode-button');
      self.bindEvents();
      self.toggleStatus();
    ...
    buildPanel: function buildPanel() {
      var self = this;
      if (!self.cache.$gutenberg.find('#tooto-switch-mode').length) {
        self.cache.$gutenberg.find('.edit-post-header-toolbar').append(self.cache.$switchMode);
      }
      if (!$('#tooto-editor').length) {
        self.cache.$editorPanel = $($('#tooto-gutenberg-panel').html());
        ...
        self.cache.$editorPanelButton = self.cache.$editorPanel.find('#tooto-go-to-edit-page-link');
        self.cache.$editorPanelButton.on('click', function (event) {
          event.preventDefault();
          self.animateLoader();

          // A new post is initialized as an 'auto-draft'.
          // if the post is not a new post it should not save it to avoid some saving conflict between tooto and gutenberg.
        ...
        if (self.istootoMode) {
          tootoCommon.dialogsManager.createWidget('confirm', {
            ...
            onConfirm: function onConfirm() {
              ...
              wpEditor.editPost({
                gutenberg_tooto_mode: false
              });
              wpEditor.savePost();
              self.istootoMode = !self.istootoMode;
              self.toggleStatus();
            }
          }).show();
        } else {
    ...
    redirectWhenSave: function redirectWhenSave() {
      ...
      setTimeout(function () {
        if (wp.data.select('core/editor').isSavingPost()) {
          ...
          location.href = tootoGutenbergSettings.editLink;
        }
      }, 300);
    },
    animateLoader: function animateLoader() {
      this.cache.$editorPanelButton.addClass('tooto-animate');
    },
    toggleStatus: function toggleStatus() {
      jQuery('body').toggleClass('tooto-editor-active', this.istootoMode).toggleClass('tooto-editor-inactive', !this.istootoMode);
    },
    init: function init() {
      this.cacheElements();
    }
