...
export default function HelloTheme() {
	...
	const activateHelloTheme = () => {
		...
		setActivateHelloThemeAjaxState( {
			data: { action: 'tooto_activate_hello_theme' },
		} );
	};
	...
	useEffect( () => {
		...
		const timeout4 = setTimeout( () => {
			...
			setActionButtonText( (
				<>
					<i className="eicon-loading eicon-animation-spin" aria-hidden="true" />
					<span className="e-onboarding__action-button-text">{ __( 'Hold on, this can take a minute...', 'tooto' ) }</span>
				</>
			) );
		}, 4000 );
		...
		const timeout30 = setTimeout( () => {
			...
			setActionButtonText( (
				<>
					<i className="eicon-loading eicon-animation-spin" aria-hidden="true" />
					<span className="e-onboarding__action-button-text">{ __( 'Okay, now we\'re really close...', 'tooto' ) }</span>
				</>
			) );
		}, 30000 );
	...
	return (
		<Layout pageId={ pageId } nextStep={ nextStep }>
			...
			>
				<p>
					{ __( 'Hello is tooto\'s official blank canvas theme optimized to build your website exactly the way you want.', 'tooto' ) }
				</p>
				<p>
					{ __( 'Here\'s why:', 'tooto' ) }
				</p>
				<ul className="e-onboarding__feature-list">
					<li>{ __( 'Light-weight and fast loading', 'tooto' ) }</li>
					<li>{ __( 'Great for SEO', 'tooto' ) }</li>
					<li>{ __( 'Already being used by 1M+ web creators', 'tooto' ) }</li>
				</ul>
			</PageContentLayout>
