...
import { Checkbox, Text } from '@tooto/app-ui';
		...
		>
			{
				props.taxonomiesByType.data.length >= MIN_TAGS_LENGTH_FOR_SEARCH_INPUT &&
					<SearchInput
						...
						placeholder={ sprintf( __( 'Search %s...', 'tooto' ), props.taxonomiesByType.label ) }
						value={ search }
						onChange={ ( searchTerm ) => {
			...
			<div className="e-kit-library__tags-filter-list-container">
				{ 0 === taxonomies.length && <Text>{ __( 'No Results Found', 'tooto' ) }</Text> }
				{
